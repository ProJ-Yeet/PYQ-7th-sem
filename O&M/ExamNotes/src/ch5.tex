...
{\color{sub}\footnotesize 5 hrs \gap$\cdot$\gap asked in \mk{25 of 29 papers}
...
\lead{The information system inside the organization, inside its environment}
\figC{c5_is_environment.png}{0.60\textwidth}
{\color{sub}\footnotesize The functions above as one loop: \mk{input} (data capture from inside and
outside), \mk{processing} (classify, arrange, calculate), \mk{output} (dissemination), and
\mk{feedback} to correct the input. Suppliers, customers, regulators, stockholders and
competitors are the external sources. Adhikari notes, p281.}
...
\lead{The pyramid, drawn: system type by level, across the functional areas}
\figC{c5_is_pyramid.png}{0.66\textwidth}
{\color{sub}\footnotesize \mk{Operational} level runs on TPS (order processing, payroll),
\mk{management} level on MIS and DSS (budgets, sales analysis), \mk{strategic} level on ESS
(5-year forecasts). Each vertical slice is one functional area. Adhikari notes, p300.}
...
\creamq{Database information system}{\m{4}\m{5}}
...
\creamq{Networking information system}{\m{4}\m{5}}

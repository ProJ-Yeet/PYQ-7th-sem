...
\lead{The motivation equation: what the member gives, and gets back}
\figC{c3_motivation_eq.png}{0.62\textwidth}
{\color{sub}\footnotesize Inputs (time, effort, skills) become \mk{performance} toward the
organization's goals, which returns \mk{outcomes} the member values (pay, security, autonomy).
Motivation holds only while the outcomes feel worth the inputs: the definition above, drawn.
Adhikari notes, p201.}

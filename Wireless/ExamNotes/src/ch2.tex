...
{\color{sub}\footnotesize 4 hrs \gap$\cdot$\gap asked in \mk{21 of 23 papers}
...
\Q{Explain the frequency reuse concept and the co-channel reuse ratio}{\m{6}\m{4}\m{3}\m{2+4}}
{\color{sub}\footnotesize \tF{4} \yr{72 Ma, \textbf{74 Bh}, \textbf{76 Bh}, \texttt{82 Ba} \m{2+4}} \gap$\cdot$\gap
74 Bh words it as ``minimizing reuse distance maximizes spectral efficiency''
\gap$\cdot$\gap 82 Ba pairs it with the channel assignment strategies below}
...
\end{itemize}}{%
\lead{Locating the co-channel cells \mk{(use the figure above)}}
\begin{itemize}
...
\creamq{Channel assignment strategies}{\m{4}}
...
\begin{itemize}
  ...
  \item GoS $=$ probability the delay exceeds $t$ seconds:
        \[ \Pr[\text{delay} > t] = \Pr[\text{delay}>0]\;e^{-(C-A)t/H} \]

...
\begin{itemize}
  ...
  \item In dB:
        \[ PL(\text{dB}) = 40\log d - \left(10\log G_t + 10\log G_r
           + 20\log h_t + 20\log h_r\right) \]
...
\[ PL(d)\,[\text{dB}] = \overline{PL}(d_0) + 10\,n_{SF}\log\!\left(\frac{d}{d_0}\right)
   + \text{FAF} + \sum \text{PAF} \]
\begin{itemize}
...
\begin{itemize}
  ...
  \item Feed it a unit impulse $\delta(t)$ and the output is
        \[ h(t) \;=\; \sum_{n=1}^{N} A_n \exp(-j\phi_n)\,\delta(t-\tau_n) \]
...
\begin{itemize}
  ...
  \item The in-phase and quadrature sums are each Gaussian, so the \textbf{envelope} is Rayleigh:
        \[ p(r) = \begin{cases}
           \dfrac{r}{\sigma^2}\exp\!\left(-\dfrac{r^2}{2\sigma^2}\right) & 0 \le r \le \infty \\[4pt]
           0 & r < 0 \end{cases} \]
...
\begin{itemize}
  ...
  \item The random components add onto a fixed phasor, so:
        \[ p(r) = \begin{cases}
           \dfrac{r}{\sigma^2}\exp\!\left(-\dfrac{r^2+A^2}{2\sigma^2}\right)

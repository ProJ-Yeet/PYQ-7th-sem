...
{\color{sub}\footnotesize Built from the frequency analysis of all 22 papers (2070--2082 BS).
...
{\color{sub}\footnotesize A topic is TOP tier when \textbf{6 or more} of the 22 papers ask it.
...
5 & \textbf{Diversity: why, and the types} & \textbf{11/22} & Space, frequency, time, polarization, angle --- one line each \\
1 & \textbf{Evolution 1G $\rightarrow$ 5G} & 10/22 & The generation table, not dates as prose \\
6 & \textbf{Vocoders} & 10/22 & Block diagram $+$ channel, formant, cepstrum, LPC \\
7 & \textbf{FHMA} & 10/22 & Draw transmitter \emph{and} receiver; fast Vs slow hopping \\
1 & Compare the generations & 9/22 & Access technique, modulation, data rate, example system \\
4 & \textbf{OFDM} & 9/22 & IFFT/FFT block diagram $+$ orthogonality $+$ PAPR \\
7 & Hybrid SSMA, and which fix near-far & 9/22 & Pick two of FCDMA, DS/FHMA, TDCDMA; give (+) and (-) \\
7 & CDMA features, (+) and limitations & 9/22 & Soft capacity, soft handoff, self-jamming, near-far \\
7 & Define multiple access $+$ compare & 9/22 & FDMA/TDMA/CDMA/SDMA in one table \\
7 & CDMA design: self-jamming, near-far & 9/22 & Name the cure: \mk{power control} at 800 Hz \\
7 & Compare FDMA, TDMA and CDMA & 9/22 & Same table again --- learn it once, use it thrice \\
8 & Regulatory issues, spectrum management & 9/22 & Allocation, pricing, licensing, tariffs, interconnection \\
\bottomrule
\end{tabularx}
...
\toprule
\hrow \thead{Ch} & \thead{Topic} & \thead{Papers} & \thead{The one thing to be able to do} \\
2 & \textbf{Handoff} & 8/22 & Process, hard Vs soft, MCHO/NCHO/MAHO, the margin $\Delta$ \\
6 & \textbf{Linear predictive coder (LPC)} & 8/22 & Block diagram $+$ the all-pole synthesis filter \\
8 & \textbf{GSM traffic and control channels} & 8/22 & The BCH / CCCH / DCCH tree, direction of each \\
3 & Outdoor models: Okumura and Hata & 7/22 & State the validity conditions, then compare the two \\
4 & Spread spectrum, DSSS and FHSS & 7/22 & Both block diagrams $+$ $PG = B_{ss}/B$ \\
5 & Combining methods for space diversity & 7/22 & Selection, feedback, maximal ratio, equal gain \\
6 & Characteristics of the speech signal & 7/22 & Which characteristic each coder family exploits \\
8 & GSM system architecture & 7/22 & Draw MS--BSS--NSS--OSS; 4 papers want the NSS alone \\
2 & Prove $Q=\sqrt{3N}$, then derive $S/I$ & 6/22 & Cosine rule $\rightarrow$ $Q$ $\rightarrow$ $S/I = Q^n/6$ \\
3 & Small-scale fading: what, and the factors & 6/22 & Multipath, speed, speed of surroundings, signal BW \\
3 & The four types of small-scale fading & 6/22 & Flat/frequency-selective from $B_c$; fast/slow from $T_c$ \\
4 & MSK and GMSK, and why GSM chose GMSK & 6/22 & $h=0.5$, $\Delta f = R_b/4$, Gaussian LPF, $BT=0.3$ \\
5 & Why equalization, and the MSE solution & 6/22 & $\hat{\boldsymbol{\omega}} = \mathbf{R}^{-1}\mathbf{p}$, training then tracking \\
5 & Linear transversal, DFE and MLSE & 6/22 & Three structures, one diagram each \\
5 & RAKE receiver & 6/22 & $M$ correlators $+$ weights $\alpha_m$; resolves multipath \\
8 & Forward and reverse IS-95 channels & 6/22 & Pilot $W_0$, sync $W_{32}$, paging $W_1$--$W_7$; \mk{reverse has no pilot} \\
\bottomrule
\end{tabularx}
...
appear in \textbf{21 or 22 of the 22 papers}, carrying \textbf{ten} TOP-tier topics between

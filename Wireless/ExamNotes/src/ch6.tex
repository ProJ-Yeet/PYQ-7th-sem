...
{\color{sub}\footnotesize 4 hrs \gap$\cdot$\gap asked in \mk{23 of 23 papers}
...
\begin{itemize}
  \item \textbf{Speech coding is the single most reliable topic in the whole paper.} Some part of
        it appears in \mk{21 of 23 papers}, and \textbf{characteristics of the speech signal}
...
\Q{Explain the characteristics of the speech signal, and how they are used in designing coders}{\m{8}\m{6}\m{4}\m{3}\m{4+4}}
{\color{sub}\footnotesize \tS{8} \yr{\textbf{72 Ash}, \textbf{77 Ch}, 73 Ma, \texttt{79 Bh},
\textbf{80 Ch}, \textbf{\texttt{82 Bh}}, \texttt{81 Ba}, \texttt{82 Ba} \m{4+4}} \gap$\cdot$\gap
...
\penalty0\vspace{3pt}
\sbs{%
\lead{Five more properties worth a line \mk{(each with the technique it justifies)}}
\begin{itemize}
  ...
  \item \textbf{Long-term (pitch) correlation}: in a \textbf{voiced} segment, samples one
        \textbf{pitch period} apart are strongly correlated, on top of the sample-to-sample
        correlation above. $\rightarrow$ justifies the \textbf{long term predictor (LTP)},
        the second predictor in \textbf{CELP} and in GSM's \mk{RPE-LTP} codec (\S6.4). \added
  \item \textbf{Silence}: a talker is silent for roughly \textbf{half} of a two-way
        conversation (pauses between words, and listening while the other party speaks).
        $\rightarrow$ justifies \textbf{VAD} (voice activity detection) and \textbf{DTX}
        (discontinuous transmission): stop sending during a pause, and the decoder fills the
        gap with synthetic \textbf{comfort noise}. \mk{Halves the average bit rate, cuts
        interference, and saves handset battery.} \added
  \item \textbf{Perceptual masking}: a loud spectral component \textbf{hides} quantization
        noise at nearby frequencies, so noise need not be uniformly low, only low
        \emph{where the ear can hear it}. $\rightarrow$ justifies \textbf{perceptual
        weighting} in CELP, which shapes the error to hide under the formant peaks. \added
\end{itemize}}{%
...
\sbs{%
\lead{The speech generation model \mk{(draw the figure above first, always)}}
\begin{itemize}
...
{\color{sub}\footnotesize \tP{2} \yr{\textbf{71 Bh}, \texttt{82 Ba}} \gap$\cdot$\gap asked as

...
A propagation numerical has appeared in \textbf{every one of the 23 papers}.
...
{\color{sub}\footnotesize \tF{5} \yr{74 Ma \m{4+4}, \textbf{\texttt{80 Bh}} \m{4+2+2},
\textbf{\texttt{82 Bh}} \m{5}, \texttt{82 Ba} \m{8}} \gap$\cdot$\gap \mk{the single most
repeated numerical in the chapter}}
...
\end{asked}
\begin{asked}{2082 Baishakh \textperiodcentered{} Q4 \textperiodcentered{} [8]}
Find the median path loss sing Okumura's model for $d = 50$ km, $h_{te} = 100$ m,
$h_{re} = 10$ m in a suburban environment. If the base station transmitter radiates an EIRP of
1 kW at a carrier frequency of 900 MHz, find the power at the receiver (assume a unity gain
receiving antenna). [Assume $A_{mu} = 43$ dB, $G_{AREA} = 9$ dB]
\end{asked}
{\color{sub}\footnotesize One set of numbers, four papers. ``EIRP oh 1 kW'' is the 82 Bh
paper's own typo, and ``path loss sing'' is 82 Ba's. \mk{82 Ba is the only one with a
unity-gain receiver}, and it does not ask for EIRP in dBm --- see Step 4.}
...
\begin{itemize}
  ...
  \item \textbf{82 Ba variant --- unity gain receiver.} That paper asks only for $P_r$, with
        $G_r = 1$, ie \textbf{0 dB}:
        \[ P_r = 60 - 155.07 + 0 = \boxed{-95.07\ \text{dBm}} = 0.311\ \text{pW} \]
        \mk{Exactly 10 dB below the 80 Bh / 82 Bh answer}, since the only change is dropping
        the 10 dB receive gain. Margin over $-104$ dBm falls to about 9 dB.
\end{itemize}
...
\par\vspace{1pt}
{\color{sub}Read the stem plot on the right carefully: it carries \textbf{three} components,
0 dB at $\tau = 0$, $-10$ dB at $\tau = 1\ \mu$s and 0 dB at $\tau = 2\ \mu$s.
\mk{The $\tau=0$ ray is the second arrowhead sitting on the 0 dB tick, drawn over the vertical
axis. Miss it and every answer below is wrong.}}
\end{asked}}{%
\figT{c3_pdp_70ma.png}
\penalty0\vspace{1pt}
{\color{sub}\footnotesize The profile as printed on the 70 Ma paper. \mk{Two arrowheads on the
vertical line: upper one is the axis, the one at 0 dB is the $\tau=0$ ray.}}}
...
2 & $0$   & 1.0 & 2.0 & 4.0 \\
\midrule
\textbf{sum} & & \textbf{2.1} & \textbf{2.1} & \textbf{4.1} \\
\bottomrule
\end{tabular}
\end{center}}{%
\begin{itemize}
  \item \[ \bar{\tau} = \frac{2.1}{2.1} = \boxed{1.000\ \mu\text{s}} \]
  \item \[ \overline{\tau^2} = \frac{4.1}{2.1} = 1.952\ \mu\text{s}^2 \]
  \item \[ \sigma_\tau = \sqrt{1.952 - (1.000)^2} = \sqrt{0.952} = \boxed{0.976\ \mu\text{s}} \]
  \item \[ B_c(90\%) = \frac{1}{50 \times 0.976\times10^{-6}} = \boxed{20.5\ \text{kHz}} \]
  \item \[ B_c(50\%) = \frac{1}{5 \times 0.976\times10^{-6}} = \boxed{204.9\ \text{kHz}} \]
  \item \textbf{Maximum excess delay} within 10 dB of the peak: peak is 0 dB, and the
        1 $\mu$s ray is exactly 10 dB down, so all three count and
        $\tau_{10\text{dB}} = 2 - 0 = 2\ \mu$s.
  \item \mk{Sanity check:} $\bar{\tau}$ falls on exactly 1 $\mu$s because the two equal 0 dB rays
        at 0 and 2 $\mu$s balance about the centre. A clean number is the sign you read the plot
        right.
\end{itemize}}

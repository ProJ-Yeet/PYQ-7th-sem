...
\end{itemize}
\penalty0\vspace{1pt}
\figC{c1_turing_test.png}{2.4in}
{\color{sub}\footnotesize \mk{Draw this: the arrangement is the
definition.} \textbf{A} is the interrogator, \textbf{B} the human respondent,
\textbf{C} the machine. The wall and the text-only channel are the two things that must
appear $-$ they are what stops the test measuring anything except behaviour. $A$ knows
one of $B$, $C$ is a machine and must say which.}
...
{\color{sub}\footnotesize \yr{\textbf{\texttt{81 Bh}} \m{3},
\textbf{80 Ch} \m{4}} \gap$\cdot$\gap
2 papers, both under 6 marks, so no tier chip \gap$\cdot$\gap

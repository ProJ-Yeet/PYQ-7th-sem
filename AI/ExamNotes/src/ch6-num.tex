...
\begin{itemize}
  \item \textbf{Why, in words}: Outlook is the only attribute with a
        \mk{pure branch} $-$ all four \emph{Overcast} days are Yes $-$
        so it removes more uncertainty than any other. Temperature removes almost none:
        its three branches are as mixed as the parent.
\end{itemize}}{%
\lead{Step 5 \gap Finish the tree \gap{\color{sub}\footnotesize(not asked, but it is two more lines and it proves the root was right)}}
\begin{itemize}
  \item \textbf{Overcast branch} $-$ 4 rows, all Yes. \mk{Leaf: Yes.}
  \item \textbf{Rainy branch} $-$ 5 rows, 2 Yes 3 No, $H=0.9710$.
  \begin{itemize}
    \item Humidity: High $\rightarrow$ 0 Yes, 3 No ($H=0$); Normal $\rightarrow$ 2 Yes,
          0 No ($H=0$). Residual 0, \mk{Gain $=0.9710$}.
    \item Windy gives 0.0200, Temperature 0.5710. So \textbf{Humidity} splits it
          perfectly: High $\rightarrow$ No, Normal $\rightarrow$ Yes.
  \end{itemize}
  \item \textbf{Sunny branch} $-$ 5 rows, 3 Yes 2 No, $H=0.9710$.
  \begin{itemize}
    \item Windy: False $\rightarrow$ 3 Yes, 0 No; True $\rightarrow$ 0 Yes, 2 No.
          Residual 0, \mk{Gain $=0.9710$}. So \textbf{Windy}:
          False $\rightarrow$ Yes, True $\rightarrow$ No.
  \end{itemize}
  \item \textbf{The tree}, three levels and five leaves:
  \begin{itemize}
    \item \code{Outlook = Overcast -> Yes}
    \item \code{Outlook = Rainy, Humidity = High -> No}
    \item \code{Outlook = Rainy, Humidity = Normal -> Yes}
    \item \code{Outlook = Sunny, Windy = False -> Yes}
    \item \code{Outlook = Sunny, Windy = True -> No}
  \end{itemize}
  \item Temperature never appears. \mk{ID3 has decided it is
        irrelevant}, which is the feature-selection side of the algorithm.
\end{itemize}}
...
\begin{itemize}
  \item \textbf{Windy $=$ False} $-$ 7 rows, all Yes. \mk{Leaf: Yes.}
  \item \textbf{Windy $=$ True} $-$ 7 rows, 2 Yes 5 No, $H=0.8631$. Split on Outlook:
  \begin{itemize}
    \item Rainy $\rightarrow$ 3 rows, all No ($H=0$).
    \item Sunny $\rightarrow$ 2 rows, all No ($H=0$).
    \item Overcast $\rightarrow$ 2 rows, all Yes ($H=0$).
    \item Residual $0$, so \mk{Gain $=0.8631$}: a perfect split.
  \end{itemize}
  \item \textbf{The tree}, four leaves and only two tests:
  \begin{itemize}
    \item \code{Windy = False -> Yes}
    \item \code{Windy = True, Outlook = Overcast -> Yes}
    \item \code{Windy = True, Outlook = Rainy -> No}
    \item \code{Windy = True, Outlook = Sunny -> No}
  \end{itemize}
...
\begin{itemize}
  \item \mk{No attribute here is much use.} The best gain is 0.0488
        against a starting entropy of 0.9544, so the root removes only about
        \textbf{5\,\%} of the uncertainty. With eight examples and four binary
        attributes the data simply does not determine a good tree $-$ and on this
        question that is worth saying, because the story is about eating something
        poisonous.
  \item \mk{Classifying U, V and W}, which the story implies even though
        the question stops at the root:
  \begin{itemize}
    \item \code{U: Smooth = 1} $\rightarrow$ the 3:1 inedible branch $\rightarrow$
          predict \textbf{not edible}.
    \item \code{V: Smooth = 1} $\rightarrow$ predict \textbf{not edible}.
    \item \code{W: Smooth = 0} $\rightarrow$ the 2:2 branch, \mk{a tie}
          $-$ the tree cannot decide, and a majority-class leaf would be a coin toss.
  \end{itemize}
  \item So the honest answer ends: \textbf{do not eat W}. A model this uncertain should
        abstain, not guess.
\end{itemize}}
...
\creamq{Explain the Mamdani fuzzy inference method / the steps of a fuzzy inference system, with an example}{\m{2+5}\m{2+6}\m{3+7}\m{8}}
{\color{sub}\footnotesize \tS{16} \yr{73 Ma \m{3+7}, \texttt{82 Ba} \m{2+6},
\textbf{\texttt{80 Bh}} \m{8}, \textbf{\texttt{79 Bh}} \m{2+6}, 70 Ma \m{2+6},
80 Ash \m{1+2+5}, 79 Ash \m{1+2+5}, \textbf{80 Ch} \m{2+5}, \textbf{69 Bh} \m{4},

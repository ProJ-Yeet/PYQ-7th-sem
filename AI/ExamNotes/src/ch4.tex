...
23 resolution proofs and 15 Bayes numericals worked in \emph{Ch4 -- Knowledge
...
all 23 distinct premise sets are worked in the companion}
...
\sbs{%
\lead{The 23 premise sets, grouped}
\begin{itemize}
  \item \textbf{Likes-all-food / peanuts} $-$ \textbf{7 papers}, five namings of one
        puzzle (John/Bill/Sue, Ram/Krishna/Radha, Ram/Anil, Sita/mushroom,
        Bhogendra/chicken). \mk{Learn this one first.}
  \item \textbf{Charlie is a horse / a mammal} $-$ 6 papers, including the
        Kali-and-Taarey cow rewording. Tests \emph{inverse relations}.
  \item \textbf{Colonel West / weapons to hostile nations} $-$ 6 papers under five names
        (John, Donald, Colonel West, Gorge, Mr.\,R). Tests \emph{existential
        instantiation} of ``has some missiles''.
  \item \textbf{Light sleeper, hounds and mice} $-$ 3 papers (hounds, direwolves). The
        hardest of the four: the goal is itself an implication, so
        \mk{negating it produces three clauses}.
  \item Then 12 one-off sets: Marcus and Caesar, Santa and Rudolph, Curiosity and the cat,
        Sneha and the lottery, Steve and BK301, the strawberry-picking day, the
        oversmart/naughty pair, cup-on-book, Mary loves a football star, the exciting
        life, and the two forward-chaining variants.
\end{itemize}}{%

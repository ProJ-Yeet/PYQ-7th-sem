...
\end{itemize}
\penalty0\vspace{1pt}
\figC{c7_es_development.png}{2.0in}
{\color{sub}\footnotesize \mk{A second, coarser framing of the same thing},
and the one BA Sir's deck gives. It is not a rival to the six stages: acquisition and
representation are stage 3, inferencing is stage 4, transfer is stage 5. \gap$\cdot$\gap
\textbf{Either list is accepted.} Give the six stages, and draw this beside them if the
question says ``with a diagram'' $-$ it is the only \emph{flow} in the answer, the block
diagram in \S7.6 being a \emph{structure}.}}

...
\begin{itemize}
  \item \textbf{Balanced (reflection-type) realisation}: carrier into the H-arm of a magic
    tee, identical PIN diodes as reflecting terminations on the two collinear arms, both
    driven by the modulating bias, and \textbf{one diode set back by $\lambda_g/4$} so its
    round-trip reflection returns $180^\circ$ shifted, $\Gamma_2 = -\Gamma_1$. A tee with
    loads $\Gamma_1, \Gamma_2$ gives $S_{44} = (\Gamma_1 + \Gamma_2)/2 = 0$ and
    $S_{34} = (\Gamma_1 - \Gamma_2)/2 = \Gamma_1$ (Ch4 $\S$4.5, asserted in \code{wg.py}):
    the input stays matched and the E-arm output is $\Gamma_1(t)$, which follows the bias.
    \mk{Without the offset} ($\Gamma_1 = \Gamma_2$) every reflection returns to the
    H-arm input and nothing reaches the E-arm. \added{}

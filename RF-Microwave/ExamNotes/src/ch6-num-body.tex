...
$M$\rule[-2.3ex]{0pt}{5.4ex} & $\dfrac{|S_{11}||S_{12}||S_{21}||S_{22}|}{(1 - |S_{11}|^2)(1 - |S_{22}|^2)}$ & \\
\rule[-2.3ex]{0pt}{5.4ex} & $= \dfrac{0.55\times0.04\times2.82\times0.45}{0.6975\times0.7975}$ & $0.0502$ \\
Error bound\rule[-2.3ex]{0pt}{5.4ex} & $\dfrac{1}{(1 + M)^2} < \dfrac{G_T}{G_{TU}} < \dfrac{1}{(1 - M)^2}$ & \\
\rule[-2.3ex]{0pt}{5.4ex} & $\dfrac{1}{(1.0502)^2} < \dfrac{G_T}{G_{TU}} < \dfrac{1}{(0.9498)^2}$ & $-0.43$ to $+0.45$ dB \\
\bottomrule
\end{tabularx}
\lead{Step 7: compare}
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Job of each network}: turn a $50\,\Omega$ termination into the $\Gamma$ the
    transistor needs. From the gate looking back to the source it must see
    $\Gamma_S = 0.6307\angle147.96^\circ$; from the drain looking to the load,
    $\Gamma_L = 0.5551\angle26.74^\circ$ (Step 4).
  \item \textbf{Stub first}: at the $50\,\Omega$ end $y = 1$, the chart centre. A shunt stub adds
    only susceptance, $y = 1 + jb$, so the point slides along the $g = 1$ circle. Choose $b$ so it
    lands on the circle $|\Gamma| = |\Gamma_S|$.
  \item \textbf{Line second}: a lossless $50\,\Omega$ line keeps $|\Gamma|$ and only rotates it
    clockwise (away from the stub, towards the device) by $2\beta l$, i.e. $720^\circ$ per
    $\lambda$. It fixes the angle.
  \item \textbf{Finding $b$}: $\Gamma = \dfrac{1 - y}{1 + y} = \dfrac{-jb}{2 + jb}$
    $\Rightarrow$ $|\Gamma|^2 = \dfrac{b^2}{4 + b^2}$ $\Rightarrow$
    $b = \dfrac{2|\Gamma|}{\sqrt{1 - |\Gamma|^2}}$, with $\angle\Gamma = -90^\circ - \tan^{-1}(b/2)$.
  \item \textbf{Lengths}: open stub $b = \tan\beta l_s$ $\Rightarrow$
    $l_s = \dfrac{\tan^{-1}b}{360^\circ}\lambda$; line
    $l = \dfrac{\angle\Gamma_{stub} - \angle\Gamma_{target}}{720^\circ}\lambda$ (add $360^\circ$
    to the numerator if negative).
\end{itemize}
\begin{tabularx}{\linewidth}{@{}L{2.5cm}L{6.0cm}X@{}}
\toprule
\hrow \thead{Step} & \thead{Input network ($\Gamma_S$)} & \thead{Output network ($\Gamma_L$)} \\
\midrule
$|\Gamma|$, $|\Gamma|^2$ & $0.6307$, $0.3978$ & $0.5551$, $0.3081$ \\
$\sqrt{1 - |\Gamma|^2}$ & $0.7760$ & $0.8318$ \\
$b$ & $2\times0.6307/0.7760 = 1.6255$ & $2\times0.5551/0.8318 = 1.3347$ \\
Open stub & $\tan^{-1}1.6255 = 58.40^\circ$; $58.40/360 = \mathbf{0.1622\lambda}$ & $\tan^{-1}1.3347 = 53.16^\circ$; $\mathbf{0.1477\lambda}$ \\
$\Gamma$ after stub & $-90^\circ - \tan^{-1}0.8128$: $0.6307\angle{-129.10^\circ}$ & $-90^\circ - \tan^{-1}0.6673$: $0.5551\angle{-123.72^\circ}$ \\
Rotation needed & $-129.10 - (147.96 - 360) = 82.94^\circ$ & $-123.72 - (26.74 - 360) = 209.54^\circ$ \\
Line & $82.94^\circ/720 = \mathbf{0.1152\lambda}$ & $209.54^\circ/720 = \mathbf{0.2910\lambda}$ \\
\bottomrule
\end{tabularx}
\begin{itemize}
  \item \textbf{Circuit}: $50\,\Omega$ source $\rightarrow$ shunt open stub $0.1622\lambda$
    $\rightarrow$ $0.1152\lambda$ line $\rightarrow$ transistor $\rightarrow$ $0.2910\lambda$ line
    $\rightarrow$ shunt open stub $0.1477\lambda$ $\rightarrow$ $50\,\Omega$ load. Stubs sit at the
    $50\,\Omega$ ends; lines touch the transistor.
  \item $-b$ also works (input stub $0.3378\lambda$, line $0.4738\lambda$; output $0.3523\lambda$,
    $0.1347\lambda$); $+b$ gives the shorter network on both sides. Same 12.10 dB either way.
\end{itemize}
\par\penalty0\Needspace{15\baselineskip}\vspace{2pt}
\noindent\mbox{}\hfill\includegraphics[width=0.40\textwidth]{figs/c6n_p1_input.png}\hfill
\includegraphics[width=0.40\textwidth]{figs/c6n_p1_output.png}\hfill\mbox{}\par
{\color{sub}\footnotesize The table on the chart: the stub slides the $50\,\Omega$ centre along
$g = 1$ (green) onto the $|\Gamma|$ circle (dotted); the line rotates it clockwise to $\Gamma_S$ or
$\Gamma_L$. Drawn by \code{ampfig.py}, which walks each network forward to confirm it reaches
$\Gamma$ and stops if any tabled step differs.}
...
\end{itemize}
\par\penalty0\Needspace{15\baselineskip}\vspace{2pt}
\noindent\mbox{}\hfill\includegraphics[width=0.40\textwidth]{figs/c6n_p3_source.png}\hfill
\includegraphics[width=0.40\textwidth]{figs/c6n_p3_load.png}\hfill\mbox{}\par
{\color{sub}\footnotesize The two circles drawn from the table: each cuts the chart, so a
sliver of the chart (hatched) is unstable; the unilateral design points $S_{11}^*$, $S_{22}^*$
used in Step 5 sit on the stable side. Generated from the S-parameters by
\code{ampfig.py}, which stops if these centres and radii differ from the table.}
...
\begin{itemize}
  ...
  \item \textbf{``Modify if necessary''}: it is necessary, for the bilateral part. Worked in full
    after the chart.
\end{itemize}
\par\penalty0\Needspace{15\baselineskip}\vspace{2pt}
\sbsr{0.42}{%
\includegraphics[width=\linewidth]{figs/c6n_p4_circles.png}}{%
\vspace{6pt}
\begin{itemize}
  \item Both stability circles drawn on one chart, since each only \textbf{clips the rim}:
    the input circle near $166^\circ$, the output circle (radius $33.6$, so nearly a straight
    line) near $63^\circ$.
  \item Everything inside $|\Gamma| \approx 0.96$ is stable: this is why ``potentially
    unstable'' here is narrow, and why $S_{11}^*$ and $S_{22}^*$ are safe.
  \item From \code{ampfig.py}, checked against the centres and radii above.
\end{itemize}}

\par\penalty0\Needspace{12\baselineskip}
\lead{``Modify the S-parameters'': stabilise with a resistor, then redo the design}
\begin{itemize}
  \item \textbf{Why}: $B_1^2 - 4|C_1|^2 = 4|S_{12}S_{21}|^2(K^2 - 1) = -0.0300 < 0$, so both roots
    of the conjugate-match formula have $|\Gamma_S| = 1$: no bilateral $G_{T,max}$ exists. Add a
    resistor so that resistor $+$ transistor is one new two-port with $K > 1$. Its S-parameters
    are the ``modified'' ones.
  \item \textbf{Where}: the unstable source region hugs the rim at $\Gamma_S = 0.981\angle166^\circ$,
    i.e. $Z_S = 0.49 + j6.14\,\Omega$, an almost-short source. A \textbf{series} $R$ at the gate
    means the transistor never sees less than $R$ of source resistance. $K$ belongs to the whole
    two-port, so once $K > 1$ both ports are safe.
  \item Smallest that works: $R = 0.48\,\Omega$. Take \textbf{$R = 5\,\Omega$} for margin,
    $r = R/Z_0 = 0.1$.
\end{itemize}
\lead{Step A: the resistor as a two-port, cascaded in front of the transistor}
\begin{itemize}
  \item Series element $z$: $A_{11} = A_{22} = r/(2 + r) = 0.1/2.1 = 0.0476$;
    $A_{12} = A_{21} = 2/(2 + r) = 2/2.1 = 0.9524$. (Shunt $y$: $-y/(2 + y)$ and $2/(2 + y)$.)
  \item Common denominator $D = 1 - A_{22}S_{11} = 1 - 0.0476\times0.64\angle{-169^\circ}
    = 1 - 0.0305\angle{-169^\circ} = 1.0299\angle0.32^\circ$.
  \item $S_{11}' = A_{11} + A_{21}^2S_{11}/D
    = 0.0476 + (0.9070\times0.64\angle{-169^\circ})/(1.0299\angle0.32^\circ)
    = 0.0476 + 0.5636\angle{-169.32^\circ} = \boxed{0.5169\angle{-168.35^\circ}}$.
  \item $S_{12}' = A_{12}S_{12}/D = (0.9524\times0.03\angle50^\circ)/(1.0299\angle0.32^\circ)
    = \boxed{0.0277\angle49.7^\circ}$.
  \item $S_{21}' = A_{21}S_{21}/D = (0.9524\times10.11\angle91^\circ)/(1.0299\angle0.32^\circ)
    = \boxed{9.349\angle90.68^\circ}$.
  \item $S_{22}' = S_{22} + A_{22}S_{12}S_{21}/D
    = 0.22\angle{-82^\circ} + (0.0476\times0.3033\angle141^\circ)/(1.0299\angle0.32^\circ)
    = 0.22\angle{-82^\circ} + 0.0140\angle140.68^\circ = \boxed{0.2099\angle{-84.60^\circ}}$.
\end{itemize}
{\color{sub}\footnotesize These are the standard two-port cascade lines (resistor first, then
transistor); \code{amp.py} checks them against the ABCD product.}

\par\penalty0\Needspace{12\baselineskip}
\lead{Step B: stability of the modified device}
\begin{itemize}
  \item $|S_{11}'|^2 = 0.2672$, $|S_{22}'|^2 = 0.0441$, $|S_{12}'S_{21}'| = 0.2593$.
  \item $\Delta' = 0.1085\angle107.06^\circ - 0.2593\angle140.35^\circ
    = (-0.0318 + j0.1037) - (-0.1997 + j0.1655) = 0.1679 - j0.0618
    = \boxed{0.1789\angle{-20.2^\circ}}$, $|\Delta'|^2 = 0.0320$.
  \item $K' = \dfrac{1 - 0.2672 - 0.0441 + 0.0320}{2\times0.2593} = \dfrac{0.7207}{0.5187}
    = \boxed{1.390} > 1$, $|\Delta'| < 1$ $\Rightarrow$ \mk{unconditionally stable}.
  \item $|S_{21}'/S_{12}'| = 337$, unchanged (both scaled by $A_{21}/D$): the resistor does not
    touch the MSG, it raises $K$.
\end{itemize}
\par\penalty0\Needspace{12\baselineskip}
\lead{Step C: bilateral and unilateral maximum gain of the modified device}
\begin{tabularx}{\linewidth}{@{}L{2.5cm}L{6.4cm}X@{}}
\toprule
\hrow \thead{Quantity} & \thead{Formula and substitution} & \thead{Value} \\
\midrule
$B_1$, $C_1$ & $1 + 0.2672 - 0.0441 - 0.0320$; $S_{11}' - \Delta'S_{22}'^*$ & $1.1911$, $0.5405\angle{-165.18^\circ}$ \\
$B_2$, $C_2$ & $1 + 0.0441 - 0.2672 - 0.0320$; $S_{22}' - \Delta'S_{11}'^*$ & $0.7449$, $0.2759\angle{-69.12^\circ}$ \\
Discriminant & $4|S_{12}'S_{21}'|^2(K'^2 - 1)$ & $0.2504$, root $0.5004$ \\
$\Gamma_S$ & $(1.1911 - 0.5004)/(2\times0.5405\angle{-165.18^\circ})$ & $\mathbf{0.6390\angle165.18^\circ}$ \\
$\Gamma_L$ & $(0.7449 - 0.5004)/(2\times0.2759\angle{-69.12^\circ})$ & $\mathbf{0.4431\angle69.12^\circ}$ \\
$G_{T,max}$ & $1.6901\times87.40\times0.9690$ & $\mathbf{143.1 = 21.56\ dB}$ \\
Check & $337.0\times(1.3895 - 0.9648) = 337.0\times0.4247$ & $143.1$ \checkmark \\
$G_{TU,max}$ & $1.3646\times87.40\times1.0461$ & $\mathbf{124.8 = 20.96\ dB}$ \\
$M'$, bound & $0.040$ & $-0.34$ to $+0.36$ dB \\
\bottomrule
\end{tabularx}
\begin{itemize}
  \item \textbf{Answer}: as given, $K = 0.958$: potentially unstable, unilateral $22.6$ dB, no
    bilateral match (MSG ceiling $25.3$ dB). Modified with $5\,\Omega$ in series at the gate:
    $K = 1.390$, bilateral $\boxed{21.56\text{ dB}}$, unilateral $\boxed{20.96\text{ dB}}$. The
    resistor costs about 1 dB and buys a design that cannot oscillate.
  \item Networks now present $\Gamma_S$ at the resistor's outer end: $50\,\Omega$ $\rightarrow$
    open stub $0.1638\lambda$ $\rightarrow$ line $0.0904\lambda$ $\rightarrow$ $5\,\Omega$
    $\rightarrow$ gate; output stub $0.1241\lambda$, line $0.2425\lambda$ (Problem 1's table method).
  \item Other placements, checked: series $R$ at the drain needs $R > 4.02\,\Omega$; shunt at the
    drain $R < 1.76$ k$\Omega$; shunt at the gate $R < 57\,\Omega$, which bleeds off input power.
    Any is accepted if you state it and recompute $K$.
  \item ``Modify'' can also be read as setting $S_{12} = 0$. That is only the unilateral model
    above; it does not make the device stable, so it cannot answer the bilateral part.
\end{itemize}
...
\end{itemize}
\par\penalty0\Needspace{15\baselineskip}\vspace{2pt}
\noindent\mbox{}\hfill\includegraphics[width=0.40\textwidth]{figs/c6n_p6_input.png}\hfill
\includegraphics[width=0.40\textwidth]{figs/c6n_p6_output.png}\hfill\mbox{}\par
{\color{sub}\footnotesize Steps 5 and 6 on the chart. From the $50\,\Omega$ centre the open stub
moves the point along the admittance $g = 1$ circle (green) onto the $|\Gamma_S|$ or $|\Gamma_L|$
circle (dotted); the line then rotates it clockwise to the required $\Gamma$. Drawn by
\code{ampfig.py} from the same numbers, which it checks against the boxed lengths.}
...
\par\penalty0\Needspace{15\baselineskip}\vspace{2pt}
\sbsr{0.42}{%
\includegraphics[width=\linewidth]{figs/c6n_p7_circles.png}}{%
\vspace{6pt}
\begin{itemize}
  \item The sketch the question wants: the unit chart and the two given circles, each
    \textbf{cutting} it. Hatch the unstable side and say which side that is.
  \item The output circle passes through the centre: that is the $50\,\Omega$ load sitting on
    the stability boundary.
  \item Drawn by \code{ampfig.py} from the printed $C$ and $R$.
\end{itemize}}

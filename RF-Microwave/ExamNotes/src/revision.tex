...
There are 22 of them and every chapter has at least one, except Chapter 8, whose marks are
...
3 & \textbf{Why S-parameters, and S Vs h-parameters} & 13/24 & A matched load exists at every frequency; a short and an open do not \\
...
3 & Properties of the S-matrix & 8/24 & Matched, reciprocal, unitary, reference-plane phase \\
...
is tagged into Chapter 1 by ten papers and into Chapter 3 by thirteen, because the papers
...
3 & Magic tee and radar power splits & 9/24 & 3 to 5, 9 to 11 & Reading the port number instead of the sign pattern \\
4 & Two magic tees joined; terminated arms & 5/24 & 6 to 8 & Not re-deriving after termination; use \code{sparam.terminate()} \\
...
3 & Identify the device from its S-matrix & 5/24 & 1, 6 to 8, 12 & Every one of them is a magic tee except the modulator \\
...
\begin{enumerate}[label=\arabic*., leftmargin=1.6em, itemsep=1.5pt]
  ...
  \item \textbf{Why are S-parameters used in microwave network analysis? Compare with
    h-parameters.} \yr{13 papers in Ch 3, 10 in Ch 1} $\rightarrow$ $\S$3.1, $\S$1.3.
  ...
  \item \textbf{Draw a neat diagram of a magic tee and derive its S-matrix.}
    \yr{9 papers} $\rightarrow$ $\S$4.5, $\S$3.6.
  ...
  \item \textbf{State and justify the properties of the scattering matrix; analyse a
    three-port network.} \yr{8 papers} $\rightarrow$ $\S$3.3, $\S$3.5.
...
\begin{itemize}
  \item \textbf{Chapter 4}: write the six derivation steps and the characteristic-equation
    table. Then the magic tee matrix, and one two-tee junction.

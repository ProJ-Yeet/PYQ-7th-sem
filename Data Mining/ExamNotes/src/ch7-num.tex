...
{\color{sub}\footnotesize This chapter is almost entirely short notes. Across all 17 papers there is
...
{\color{sub}\footnotesize \yr{76 Ch} \gap ``Apply the PageRank algorithm using a damping factor of
0.85. A minimum of five iterations are required. Assume initial page rank of all pages is 0.25.''}
...
\end{itemize}}{%
\figT{pyq_pagerank_76ch}}

\lead{Step 3: iterate}
\begin{tabularx}{0.88\textwidth}{@{}L{2.1cm}XXXX@{}}
\toprule
\hrow \thead{Iteration} & \thead{PR(A)} & \thead{PR(B)} & \thead{PR(C)} & \thead{PR(D)} \\
0 (initial) & 0.2500 & 0.2500 & 0.2500 & 0.2500 \\
...
\begin{itemize}
  \item $PR(A) = 0.15 + 0.85(0.25) = 0.15+0.2125 = 0.3625$
  \item $PR(B) = 0.15 + 0.85(0.25/2) = 0.15+0.10625 = 0.2563$
  \item $PR(C) = 0.15 + 0.85(0.125+0.25+0.25) = 0.15+0.85(0.625) = 0.6813$
  \item $PR(D) = 0.15$
\end{itemize}
...
{\color{sub}\footnotesize Some texts write $PR(A) = \frac{1-d}{N} + d\sum\frac{PR(T_i)}{C(T_i)}$,
which makes the ranks sum to \textbf{1} instead of $N$. Know both. \added}

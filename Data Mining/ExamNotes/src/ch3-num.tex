...
\sbsr{0.5}{%
\lead{Final tree}
\includegraphics[width=\linewidth]{figs/c3n_dt_78bh.png}
{\color{sub}\footnotesize 5 leaves, depth 3, classifies all 10 training tuples correctly.
Drawn by \code{dtfig.py}, which reruns ID3 on the table and checks the gains and the tree
against this page.}}{%
...
\lead{Final tree}
\mbox{}\hfill\includegraphics[width=0.90\textwidth]{figs/c3n_dt_82bh.png}\hfill\mbox{}\par
{\color{sub}\footnotesize Drawn by \code{dtfig.py}, which reruns ID3 on the table and checks the
gains and the tree against this page.}

\sbsr{0.52}{%
\lead{Reading the tree}
\begin{itemize}
  \item 8 leaves; classifies all 14 tuples correctly.
  \item \textbf{Income} alone decides the \$0--15K band (all high risk).
  \item \textbf{Credit history} is tested twice, once under each other income band: an
    attribute used on one path can be reused on another.
  \item Only the \$15--35K, unknown-history tuples need a third test, \textbf{Debt}.
\end{itemize}}{%
...
{\color{sub}\footnotesize (a) learning: training data $\rightarrow$ algorithm $\rightarrow$ model;
(b) classification: model checked on test data, then applied to new data. The precision--recall
part is Ch 3 theory, \S3.7. H\&K Fig 8.1.}

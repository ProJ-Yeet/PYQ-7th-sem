...
{\color{sub}\footnotesize \yr{79 Bh} \gap ``Draw decision tree for the given data using ID3 algorithm.''
14 tuples, attributes Age / Income / Student / Credit\_Rating, class Buy's\_Computer.}
...
{\color{sub}\footnotesize \yr{78 Bh} \gap ``Construct a decision tree for the following data set using
information gain. Predict the class label for $<$Female, 2, standard, high$>$.''
10 tuples: Gender / Car ownership / Travel cost / Income level $\rightarrow$ Transport mode.}
...
{\color{sub}\footnotesize \yr{75 Ch, 71 Ch} ask for Gini ``with suitable example'' --- use this. \added}
...
{\color{sub}\footnotesize \yr{72 Ch} \gap Predict for
...
{\color{sub}\footnotesize \yr{79 Bh} \gap Classify
$X =$ (Home Owner $=$ No, Marital Status $=$ Married, Annual Income $=$ \$120K).
10 tuples, class $=$ Defaulted Borrower.}
...
{\color{sub}\footnotesize \yr{80 Bh} \gap Classify the flower
(SepalLength $=$ 7, SepalWidth $=$ 3.2, PetalLength $=$ 4.7, PetalWidth $=$ 1.4), $K=3$, Euclidean.}
...
{\color{sub}\footnotesize \yr{74 Ch} \gap Rows $=$ \textbf{Actual}, columns $=$ Predicted.
142 / 40 / 98 / 720.}
\begin{itemize}
...
{\color{sub}\footnotesize \yr{78 Bh} \gap \mk{Same four numbers as 5.1, but rows $=$ Predicted.}
Calculate Accuracy, TPR, FPR, Precision.}
\begin{itemize}
...
{\color{sub}\footnotesize \yr{75 Ash} (rows $=$ Actual) and \yr{72 Ch} (rows $=$ predicted).
Both print 21 / 6 / 7 / 41.}
...
{\color{sub}\footnotesize \yr{74 Ash} \gap Rows $=$ Actual. 25 / 9 / 4 / 31.}
\begin{itemize}
...
{\color{sub}\footnotesize \yr{80 Ba} \gap Rows $=$ Predicted, columns $=$ Actual. 1050 / 250 / 150 / 950.
Calculate classification error, sensitivity, false alarm rate, specificity.}
\begin{itemize}
...
{\color{sub}\footnotesize \yr{73 Shr} \gap $2\times2$ table with A $=$ Yes, B $=$ No.
20 / 5 / 10 / 40. Calculate TPR, FPR, precision.}
\begin{itemize}
...
{\color{sub}\footnotesize \yr{70 Ch} \gap Build the confusion matrix from sequences.
Actual \code{A A B B B A A A B B}; X predicts the same; Y predicts \code{A A A A B B A A A B}.}

\lead{Step 1: tabulate Y position by position} (take A as the positive class)
...
{\color{sub}\footnotesize \yr{76 Ch} \gap 3 inputs $x_1,x_2,x_3$ $\rightarrow$ 2 hidden nodes
$h_1,h_2$ $\rightarrow$ output $o$. Weight $x_i \rightarrow h_j$ is $w_{ji}$;
weight $h_i \rightarrow o$ is $u_i$. Sigmoid activations, squared-error loss.}

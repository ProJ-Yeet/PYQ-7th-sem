...
{\color{sub}\footnotesize 9 hrs \gap$\cdot$\gap in 23/23 papers \gap$\cdot$\gap 5 topics, 3 TOP tier
...
{\color{sub}\footnotesize \tS{13} \yr{81 Ba, 80 Bh, 79 Bh, 75 Ch, 74 Ash, 73 Ch, 73 Shr, 72 Ch, 72 Ka, 71 Ch, 71 Shr, 70 Asa, 69 Ch}}
...
\end{itemize}
\lead{Prototype-based, found by K-means iterating (\S5.2)}
\figT{c5_kmeans_iter.png}
{\color{sub}\scriptsize (a) initial clustering around the seeds; (b) centroids recomputed and
points reassigned; (c) final, once the assignment stops changing. H\&K Fig 10.3.}}{%
\begin{itemize}
...
\end{itemize}
\lead{Grid-based, in 3 steps \pillo{gd}{NEW}}
\begin{itemize}
  \item \textbf{Partition} the data space into a multi-dim grid of finite cells.
  \item \textbf{Compute cell statistics}: count, mean, density per cell.
  \item \textbf{Merge} adjacent cells above a density threshold into clusters.
  \item \mk{Why it is the fast one}: cost depends on the \textbf{number of cells}, not the number
        of points $\rightarrow$ scales to huge datasets. Price paid: resolution is capped by
        cell size.
\end{itemize}}
...
{\color{sub}\footnotesize \tS{13} \yr{82 Bh, 81 Bh, 81 Ba, 80 Ba, 79 Ba, 78 Bh, 76 Ch, 75 Ash, 74 Ch, 74 Ash, 73 Ch, 71 Shr, 69 Ch}
...
{\color{sub}\footnotesize \tF{5} \yr{80 Bh, 80 Ba, 79 Ba, 76 Ch, 70 Ch} \gap \mk{4 of the 5 are numericals}
...
\end{itemize}
\figT{c5_agglo_divisive.png}
{\color{sub}\scriptsize AGNES merges $a..e$ upward in steps 0 to 4; DIANA splits them downward
in the reverse order. H\&K Fig 10.6.}}{%
...
\end{itemize}
\mbox{}\hfill\includegraphics[width=0.72\linewidth]{figs/c5_dendrogram_hk.png}\hfill\mbox{}\par
{\color{sub}\scriptsize The same merges as a dendrogram, level $l=0$ (all apart) to $l=4$ (one
cluster); cut at any level for that many clusters. H\&K Fig 10.7.}}
...
{\color{sub}\footnotesize \tS{12} \yr{82 Bh, 81 Bh, 79 Bh, 79 Ba, 78 Bh, 76 Ash, 75 Ch, 75 Ash, 74 Ash, 72 Ka, 70 Ch, 70 Asa}}
...
\end{itemize}
\figT{c5_dbscan_reach.png}
{\color{sub}\scriptsize Circles are $\varepsilon$-neighbourhoods, MinPts $=3$. Left: $m, p$ are core,
$q$ is density-reachable from $p$ through $m$, but not the reverse ($q$ is a border point).
Right: $r$ and $s$ are density-connected through $o$. H\&K Fig 10.14.}}{%
\lead{Algorithm}
\begin{enumerate}
...
{\color{sub}\footnotesize \tF{6} \yr{80 Bh, 74 Ch, 72 Ka, 71 Ch, 70 Asa, 69 Ch}}
...
\begin{itemize}
  \item \mk{N} Core, border and noise points $+$ \textbf{DBSCAN on 8 points}, $\varepsilon = 2$, MinPts $= 2$. \hfill \m{3+5} \yr{82 Bh} \added
  \item \mk{N} K-means, 2 clusters from a 6-instance dataset. \hfill \m{2+8} \yr{82 Bh}
  \item \mk{N} K-means from \mk{a single given centroid} $(2,3)$, $k=2$, $+$ problems and fixes. \hfill \m{5+3} \yr{81 Bh}
  \item \mk{N} K-means with the \textbf{Manhattan} distance, 3 centres, \mk{stop after 3 iterations}. \hfill \m{8} \yr{79 Ba} \added
  \item \mk{N} Clustering vs classification $+$ 1-D K-means on 7 points $+$ \textbf{compute SSE}. \hfill \m{4+8} \yr{73 Ch} \added
  \item Hierarchical clustering w/ a dendrogram example. \hfill \m{6} \yr{79 Ba}
  \item What are density-based clusters? $+$ DBSCAN w/ example. \hfill \m{10} \yr{70 Asa}
  \item Measures that can be used to \textbf{compare two clusters} \yr{70 Asa} $\cdot$ issues in cluster validation \yr{69 Ch}. \hfill \m{6}\m{6}
  \item K-means w/ examples \yr{69 Ch} $\cdot$ short notes \textbf{DBSCAN clustering} \yr{81 Bh, 79 Ba}. \hfill \m{10}\m{4}\m{4}
\end{itemize}

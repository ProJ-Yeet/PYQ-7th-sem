...
{\color{sub}\footnotesize 2 hrs \gap$\cdot$\gap in 22/23 papers \gap$\cdot$\gap 2 topics, both TOP tier
\gap$\cdot$\gap no numericals in this ch.}

\lead{Whole chapter in 2 items:}
\begin{itemize}
  \item KDD process \tS{9}
  \item DW fundamentals \tS{12}
\end{itemize}
...
{\color{sub}\footnotesize \tS{9} \yr{82 Bh, 79 Ba} \gap Most repeated Q in the paper.}
...
\penalty0\vspace{3pt}
\creamq{``Data mining turns a large collection of data into knowledge''. Justify}{\m{6}}
{\color{sub}\footnotesize \yr{81 Bh} \gap \mk{The mirror image of the question above}, and it is
asked separately, so learn it as its own answer. That one asks why we are \emph{stuck}; this one
asks how DM \emph{gets us out}. \mk{Structure the answer on the four-rung ladder} and carry one
example all the way up it. \added}

\sbs{%
\lead{The ladder, one rung at a time}
\begin{enumerate}
  \item \textbf{Data}: raw recorded facts, no context.
        \emph{Eg} 2 million supermarket receipt lines.
  \item \textbf{Information}: data aggregated and given context by querying.
        \emph{Eg} ``nappies sold 4,200 units last month''. A DW and OLAP get you here, and
        \mk{this is as far as a query tool can go} --- it answers only what you thought to ask.
  \item \textbf{Knowledge}: patterns that hold across the data and \mk{were not asked for}.
        \emph{Eg} ``customers buying nappies on Friday evening also buy beer''. Only \textbf{data
        mining} produces this rung, because only it searches the space of possible patterns.
  \item \textbf{Decision / wisdom}: acting on the pattern. \emph{Eg} move the beer display beside
        the nappies, and measure the lift in basket value.
\end{enumerate}}{%
\lead{Why the step to knowledge is data mining's alone}
\begin{itemize}
  \item \mk{Queries are deductive, mining is inductive.} SQL retrieves what you specify; mining
        induces a general rule from the instances.
  \item It is \textbf{automatic and exhaustive}: Apriori tests every itemset the thresholds allow,
        far more hypotheses than an analyst would think to write.
  \item The output is \textbf{predictive}, not just descriptive: a classifier built on past data
        labels tuples that do not exist yet.
  \item The patterns are required to be \mk{valid, novel, useful and understandable} --- Fayyad's
        four conditions. \mk{``Novel'' is what makes it knowledge}: a pattern you already knew is
        information, not knowledge.
\end{itemize}
\lead{Close with the qualifier}
\begin{itemize}
  \item DM does not turn \emph{all} data into knowledge. Garbage in, garbage out: without
        preprocessing (Ch 2) it turns noise into confident nonsense, and without pattern
        evaluation (\S4.1) it reports rules that are strong but false.
\end{itemize}}
...
{\color{sub}\footnotesize \tS{12} \yr{73 Ch, 70 Asa, 69 Ch}}
...
{\color{sub}\footnotesize \tP{3} \yr{75 Ash, 73 Ch} \gap Asks \textbf{both} directions. \yr{73 Ch} asks it as DW vs \textbf{RDBMS}.}
...
{\color{sub}\footnotesize \tP{3} \yr{70 Ch, 71 Shr, 69 Ch}}
...
{\color{sub}\footnotesize Tiers: \tS{} $=$ 9+/23 papers \gap \tF{} $=$ 5--8 \gap \tP{} $=$ 1--4 but $\ge$8 marks.
...
\begin{itemize}
  \item What is data mining? Describe the steps involved in it. \hfill \m{2+6} \yr{82 Bh}
  \item ``Data mining turns a large collection of data into knowledge''. Justify. \hfill \m{6} \yr{81 Bh} \added
  \item KDD process $+$ \mk{the challenges that motivated data mining}. \hfill \m{3+3} \yr{79 Ba}
  \item How is a data warehouse different from an \textbf{RDBMS}? Also list the similarities. \hfill \m{2+2} \yr{73 Ch}
  \item Short notes: \textbf{Data warehouse} \yr{70 Asa} $\cdot$ \textbf{Datamart} \yr{69 Ch}. \hfill \m{5}\m{5}
\end{itemize}
...
\lead{Numericals}
\begin{itemize}
  \item \mk{Ch 1 has no numerical component.} No Q in 23 papers needs a calculation here

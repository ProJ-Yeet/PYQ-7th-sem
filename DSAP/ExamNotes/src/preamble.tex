% =====================================================================
%  DSAP / DSP — Exam Notes : shared preamble
%  Lifted verbatim from Wireless\ExamNotes\src\preamble.tex; only the
%  tier-chip denominators and the running header differ.
%  45 papers here (CT 704 x28 + EX 753 x17), so the chips read /45:
%    TOP  asked in >=10 of the 45 papers
%    HOT  4-9 papers
%    PIN  1-3 papers
...
\newcommand{\tS}[1]{\pill{mkc}{white}{TOP\, #1/45}}            % >=10 papers
\newcommand{\tF}[1]{\pill{acc}{white}{HOT\, #1/45}}            % 4-9 papers
\newcommand{\tP}[1]{\pill{pin}{white}{PIN\, #1/45}}            % 1-3 papers
...
% ---------------------------------------------------------------- abbreviations
% The abbreviation chart is written as a LITERAL tabularx in abbrev.tex.
% There used to be an abbrtable environment here; it cannot work. tabularx
% re-reads its own body by scanning the token stream for the literal
% \end{tabularx}, and an environment wrapper hides that token behind
% \end{abbrtable}, so \TX@get@body runs to the end of the file and the
% build dies with "File ended while scanning use of \TX@get@body". The same
% rule is why a tabularx must never appear inside an \sbs column: use a
% plain tabular there.
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\color{sub} Digital Signal Analysis and Processing \textperiodcentered{} CT 704 / EX 753}

...
Design a low pass discrete time Butterworth filter using bilinear transformation having
following specifications: Passband frequency $(W_p)=0.25\pi$ radians. Stopband frequency
$(W_s)=0.55\pi$ radians. Passband ripple $(\delta_P)=0.11$. Stopband ripple
$(\delta_S)=0.21$. Consider sampling frequency of 0.5 Hz. Also, \mk{covert} the obtained
digital low pass filter to high pass filter with new pass band frequency,
$W'_P=0.45\pi$ using digital domain transformation.\\
\textit{71 Ch and 69 Ch print the same filter, each opening} "What is the advantage of
bilinear transformation?"\textit{, and spell "convert".}
\end{asked}
...
\par\smallskip
{\footnotesize
\begin{tabular}{@{}L{2.2cm}L{6.5cm}L{1.1cm}L{1.1cm}L{0.5cm}L{1.4cm}L{2.4cm}@{}}
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
\textbf{80 Bh} \m{12}, \textbf{81 Bh} \m{12+3} & \textbf{80 Bh}: Using
  Bilinear transformation, design a Butterworth low pass filter which satisfies
  following conditions: $0.9\le|H(e^{jw})|\le1$, for $0\le w\le\pi/2$;
  $|H(e^{jw})|\ 0.2$, for $3\pi/4\le w\le\pi$. Consider sampling frequency of 1 Hz.
  \newline {\color{sub}\mk{the second line is missing its $\le$}} \newline
  \textbf{81 Bh} prints the same filter and adds "Compare Impulse Invariance Method and
  Bilinear Transformation method"
  & $0.9151$ & $13.9794$ & 3 & $2.54674$
  & $1{+}0.43938z^{-1}$ \newline $+0.38450z^{-2}$ \newline $+0.041621z^{-3}$ \\
\hrow 81 Ba \m{2+10} & Differentiate between bilinear transformation and impulse
  invariance. Design a Butterworth digital IIR lowpass filter using bilinear
  transformation \mk{by taking $T=0.1$ second}, to satisfy the following
  specifications. $0.6\le|H(e^{j\omega})|\le1$, $0\le\omega\le0.35\pi$;
  $|H(e^{j\omega})|\le0.1$, $0.7\pi\le\omega\le\pi$ \newline {\color{sub}$T$ cancels in
  the bilinear map, so $H(z)$ is the same for any $T$; say so in a line}
  & $4.4370$ & $20$ & 2 & $1.06140$
  & $1{-}0.70699z^{-1}$ \newline $+0.26136z^{-2}$ \\
\textbf{75 Ch} \m{10}, \textbf{70 Ch} \m{10}, 82 Ba \m{12+3} & Using bilinear
  transformation, design a digital filter using Butterworth approximation which
  satisfies the following conditions $0.8\le|H(e^{jw})|\le1$ for $0\le w\le0.2\pi$;
  $|H(e^{jw})|\le0.2$ for $0.6\pi\le w\le\pi$ \newline {\color{sub}82 Ba adds "Consider
  sampling frequency 1 Hz" and a comparison of the two methods}
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
\hrow \textbf{\texttt{78 Ch}} \m{10} & Using Bilinear transformation, design a
  Butterworth filter which satisfies the following conditions:
  $0.82\le|H(e^{jw})|\le1$; $0\le\omega\le0.22\pi$; $|H(e^{jw})|\le0.18$;
  $0.58\pi\le\omega\le\pi$
  & $1.7237$ & $14.8945$ & 2 & $0.86185$
  & $1{-}0.90725z^{-1}$ \newline $+0.32103z^{-2}$ \\
73 Shr \m{12}, \textbf{\texttt{77 Ch}} \m{10}, \textbf{\texttt{76 Bh}} \m{1+10}
  & \textbf{73 Shr}: Using bilinear transformation, design a butterworth low pass filter
  which satisfies the following Magnitude Response. $0.89125\le|H(e^{jw})|\le1$ for
  $0\le\omega\le0.2\pi$; $|H(e^{iw})|\le0.17783$ for $0.3\pi\le\omega\le\pi$ \newline
  {\color{sub}\mk{$0.89125$ is 1 dB and $0.17783$ is 15 dB}; 77 Ch and 76 Bh print the
  same filter in decibels instead}
  & $1$ & $15$ & 6 & $0.72729$
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
80 Ba \m{11} & Design a low pass digital IIR filter by Billinear
  Tranformation method to an approximate Butterworth low pass filter, if passband edge
  frequency is $0.26\pi$ radians and maximum deviation of $0.99$ dB below 0 dB gain in
  the passband. The maximum gain of $-14.99$ dB and frequency is $0.58\pi$ radians in
  stopband, Consider sampling frequency 0.5 Hz. \newline {\color{sub}"Billinear
  Tranformation", as printed}
  & $0.99$ & $14.99$ & 3 & $1.08611$
  & $1{-}1.06340z^{-1}$ \newline $+0.63628z^{-2}$ \newline $-0.12114z^{-3}$ \\
\hrow 79 Ba \m{11+3} & Design a low pass digital IIR filter by Bilinear Transformation
  method to an approximate Butterworth low pass filter, if passband edge frequency is
  $0.24\pi$ radians and maximum deviation of $0.98$ dB below 0 dB gain in the passband.
  The maximum gain of $-14.95$ dB and frequency is $0.57\pi$ radians in stopband,
  consider sampling frequency 0.5 Hz. Compare impulse invariance method with bilinear
  transformation method.
  & $0.98$ & $14.95$ & 3 & $0.99560$
  & $1{-}1.19710z^{-1}$ \newline $+0.71861z^{-2}$ \newline $-0.14406z^{-3}$ \\
\textbf{\texttt{79 Ch}} \m{15}, \textbf{\texttt{72 Ash}} \m{12} & Design a low pass
  discrete IIR filter by Bilinear Transformation method to an approximate Butterworth
  filter, if pass band edge frequency $(\omega_p)=0.24\pi$ radians and maximum deviation
  of 1 dB below 0 dB gain in the pass band. The maximum gain of $-14.9$ dB and Stop band
  edge frequency $(\omega_s)=0.57\pi$ radians. Consider the sampling frequency of 0.5 Hz.
  & $1$ & $14.9$ & 3 & $0.99186$
  & $1{-}1.20280z^{-1}$ \newline $+0.72232z^{-2}$ \newline $-0.14509z^{-3}$ \\
\bottomrule
\end{tabular}}
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
\hrow \textbf{\texttt{73 Bh}} \m{11} & Design a low pass digital filter by Bilinear
  Transformation method to an approximate Butterworth filter, if passband edge frequency
  is $0.25\pi$ radians and maximum deviation of $0.99$ dB below 0 dB gain in the
  passband. The maximum gain of $-14.85$ dB and frequency is $0.59\pi$ radians in
  stopband, Consider sampling frequency 0.5 Hz.
  & $0.99$ & $14.85$ & 3 & $1.03961$
  & $1{-}1.13140z^{-1}$ \newline $+0.67685z^{-2}$ \newline $-0.13245z^{-3}$ \\
\textbf{\texttt{71 Bh}} \m{9} & Design a low pass digital filter by Bilinear
  Transformation method to an approximate Butterworth filter, if passband edge frequency
  is $0.25\pi$ radians and maximum deviation of 1 dB below 0 dB gain in the passband.
  The maximum gain of $-15$ dB and frequency is $0.55\pi$ radians in stopband, consider
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
72 Ka \m{15} & Design a low pass digital filter by Bilinear
  Transformation method to an approximate Butterworth filter, if passband edge frequency
  is $0.25\pi$ radians and maximum deviation of 1 dB below 0 dB gain in the passband.
  The maximum gain of $-15$ dB and frequency is $0.45\pi$ radians in stopband, Consider
  sampling frequency 1Hz. \newline {\color{sub}\mk{the only one of these that needs
  $N=4$}: a $0.2\pi$ transition rather than $0.3\pi$}
  & $1$ & $15$ & 4 & $0.98086$
  & $1{-}1.64760z^{-1}$ \newline $+1.35630z^{-2}$ \newline $-0.52524z^{-3}$ \newline
    $+0.083384z^{-4}$ \\
\hrow 71 Shr \m{15} & Design a low pass digital filter by Bilinear Transformation method
  to an approximate Butter worth filter it passband frequency is $0.2\pi$ radians and
  maximum deviation of 1 db below 0 dB gain in the pass band. The maximum gain of $-15$
  db and frequency is $0.4\pi$ radians in stop band, consider sampling frequency 1 Hz.
  & $1$ & $15$ & 3 & $0.81397$
  & $1{-}1.48260z^{-1}$ \newline $+0.92964z^{-2}$ \newline $-0.20333z^{-3}$ \\
\bottomrule
\end{tabular}}
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
\texttt{73 Ma} \m{11} & Design a low pass Butterworth filter using bilinear method to
  meet the following specifications: Maximum deviation of $0.98$dB below 0dB gain in the
  pass band. Stop band attenuation $=20$dB. Pass band edge frequency $=0.2\pi$ radian.
  Stop band edge frequency $=0.5\pi$ radian. Sampling frequency $=1$Hz.
  & $0.98$ & $20$ & 3 & $0.81704$
  & $1{-}1.47760z^{-1}$ \newline $+0.92551z^{-2}$ \newline $-0.20214z^{-3}$ \\
\hrow \textbf{79 Bh} \m{11+4}, \textbf{71 Ch} \m{2+9+4}, \textbf{69 Ch} \m{2+9+4}
  & worked in full above, from 79 Bh's own wording
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
76 Ash \m{12} & Design a low pass discrete IIR filter by Bilinear
  Transformation method to an approximate Butterworth filter having specifications as
  below: Pass bandedge frequency $(\omega_p)=0.22\pi$ radians. Stop bandedge frequency
  $(\omega_s)=0.54\pi$ radians. Passband ripple $(\delta_p)=0.11$. Stopband ripple
  $(\delta_s)=0.22$, Consider sampling frequency 0.5 Hz.
  & $1.0122$ & $13.1515$ & 2 & $1.00598$
  & $1{-}0.76056z^{-1}$ \newline $+0.27575z^{-2}$ \\
\hrow 74 Ash \m{15} & Design a low pass discrete IIR filter by Bilinear Transformation
  method to an approximate Butterworth filter having specifications as below: Pass
  bandedge frequency $(\omega_p)=0.27\pi$ radians. Stop bandedge frequency
  $(\omega_s)=0.58\pi$ radians. Passband ripple $(\delta_p)=0.11$. Stopband ripple
  $(\delta_s)=0.21$, Consider sampling frequency 0.5 Hz.
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$\alpha_p$} & \thead{$\alpha_s$} & \thead{$N$} & \thead{$\Omega_c$} & \thead{$H(z)$ denominator} \\
\textbf{68 Bh} \m{18} & If passband edge frequency $\omega_p=0.25\pi$, stopband edge
  frequency $\omega_s=0.45\pi$, passband ripple $\delta_p=0.17$ and stopband ripple
  \mk{$\delta_p=0.27$} then design a digital lowpass Butterworth filter using bilinear
  transformation technique. \newline {\color{sub}\mk{it prints $\delta_p$ twice}; the
  second is plainly $\delta_s$}
  & $1.6184$ & $11.3727$ & 3 & $0.94579$
  & $1{-}1.27310z^{-1}$ \newline $+0.77005z^{-2}$ \newline $-0.15836z^{-3}$ \\
\hrow 75 Ash \m{12}, \textbf{\texttt{74 Bh}} \m{9} & Design a digital low-pass filter
  with the following specification: i) Pass-band magnitude constant to $0.7$ dB below
  the frequency of $0.15\pi$ ii) Stop-band attenuation at least 14 dB for the
  frequencies between $0.6\pi$ to $\pi$. Use Butterworth approximation as a prototype
  and use bilinear transformation method to obtain the digital filter. \newline
  {\color{sub}\mk{74 Ch prints this word for word but says impulse invariance}, which
  is why it lands in \S2 and not here}
  & $0.7$ & $14$ & 2 & $0.74249$
  & $1{-}1.03700z^{-1}$ \newline $+0.36853z^{-2}$ \\
\bottomrule
\end{tabular}}
...
techniques for the given specifications: Passband peak to peak ripple $\le$ 1dB.
Passband edge frequency $=1.2$KHz. Stopband Attenuation $\ge$ 40dB. Stopband edge
frequency $=2.5$ KHz. Sample rate $=8$KHz.
\end{asked}
...
Design a digital low pass butterworth filter by applying bilinear transformation
technique for the given specifications: Pass band edge $=120$ Hz. Pass band attenuation
$=1$ dB. Stop band edge $=170$ Hz. Stop band attenuation $=16$ dB. Assume sampling
frequency of 256 Hz.
\end{asked}
\begin{itemize}
...
Design a digital Butterworth low pass filter satisfying the constraints
$0.707\le|H(e^{jw})|\le1$, $0\le w\le\pi/2$; $|H(e^{jw})|\le0.2$, $3\pi/4\le w\le\pi$
With $T=1$sec using bilinear transformation method. Realize the filter using the most
convenient realization form.
\end{asked}
\begin{itemize}
...
Design a digital low-pass filter with the following specification: i) Pass-band
magnitude constant to $0.7$ dB below the frequency of $0.15\pi$ ii) Stop-band
attenuation at least 14 dB for the frequencies between $0.6\pi$ to $\pi$. Use Butterworth
approximation as a prototype and use \mk{bilinear transformation} method to obtain the
digital filter.\\
\textit{74 Ch prints the identical specification and asks for} \mk{impulse invariance}
\textit{instead, which is the only difference between them and the reason it is worked
in \S2.}
\end{asked}
...
Find the system function for digital filter using \mk{impulsive invariance} technique
from the analog Butterworth filter transfer function
$H(s)=\dfrac{1}{(s+1.3)(s-1.3e^{j2\pi/3})(s-1.3e^{-j2\pi/3})}$ $T=1$ second, and draw
the block diagram of the system function, $H(z)$, realized in terms of second order
sections.
\end{asked}
\begin{itemize}
...
Design the \mk{Chebysev} filter using bilinear transformation to meet the following
specification. $0.707\le|H(e^{jw})|\le1$; for $|w|\le0.2\pi$. $|H(e^{jw})|\le0.1$; for
$0.5\pi\le|w|\le\pi$
\end{asked}
...
\par\smallskip
{\footnotesize
\begin{tabular}{@{}L{1.5cm}L{5.6cm}L{1.2cm}L{0.5cm}L{2.9cm}L{3.2cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed} & \thead{$\varepsilon$} & \thead{$N$}
& \thead{Poles} & \thead{$H(z)$} \\
\texttt{74 Ma} & Design a low pass digital filter by bilinear transformation method to
  an approximate Chebyshev filter for passband edge $0.2\pi$ with 1 dB ripple and
  $-15$ dB at $0.3\pi$, with $T=1$.
  & $0.50885$ & 4 & $-0.0907\pm j0.6390$ \newline $-0.2189\pm j0.2647$
  & $K=0.0018356$, \newline numerator $K(1+z^{-1})^4$, \newline denominator
    $1-3.054340z^{-1}$ \newline $+3.828999z^{-2}$ \newline $-2.292452z^{-3}$ \newline
    $+0.550745z^{-4}$ \\
\hrow \texttt{70 Ma} & Design a low pass digital filter by bilinear transformation to an
  approximate Chebyshev filter with $\omega_p=0.25\pi$, $\omega_s=0.55\pi$,
  $\alpha_{\max}=1.01$ dB, $\alpha_{\min}=13.55$ dB and $f_s=0.5$ Hz.
...
Design an analog filter using Chebyshev approximation which has maximum passband
attenuation of 5dB at frequency 20 rad/sec and stopband attenuation of 20dB at frequency
50 rad/sec. Taking sampling frequency of 0.5 Hz convert the obtained analog filter to
digital by using Bilinear transformation method.
\end{asked}
\begin{itemize}

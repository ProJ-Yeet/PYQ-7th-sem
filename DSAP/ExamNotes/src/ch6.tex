...
\begin{itemize}
  ...
  \item \mk{The sampling period cancels.} When the spec is given in rad/sample, pre-warp
        by $\Omega=\frac{2}{T}\tan\frac{\omega}{2}$ and map back by $s=\frac{2}{T}\cdots$
        and \textbf{every $2/T$ divides out}: $H(z)$ is the same for $T=1$, $T=2$ or
...
\end{tabular}
\par\smallskip
{\color{sub}\footnotesize The standing example: $\Omega_p=1$, $\Omega_s=3.33$,
$\alpha_p=0.3$ dB, $\alpha_s=22$ dB needs \textbf{$N=5$ Butterworth} but only
\textbf{$N=3$ Chebyshev}. On the 45 papers' own recurring spec ($0.2\pi$, $0.3\pi$,
1 dB, 15 dB) it is \mk{$N=6$ against $N=4$}. That single sentence answers "compare"

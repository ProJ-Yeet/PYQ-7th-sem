...
\texttt{74 Ma} \m{5} & Find the response of an \mk{FIR system} with input
 ...
 {\color{sub}$h$ and $x$ are named the other way round from every sibling}
 & $\{\underline{0},10,26,50,38,32,12,0\}$ \\
\hrow \textbf{71 Ch} \m{6} & A discrete time LTI system has impulse response
 $h(n)=\{1,3,2,-1,1\}$ \mk{for $-1\le n\le3$}. Determine the system output $y(n)$ if the
 input $x(n)$ is given by $x(n)=2\delta(n)-\delta(n-1)$.
 & $\{2,\underline{5},1,-4,3,-1\}$, from $n=-1$ \\
\textbf{79 Bh}, \textbf{81 Bh} \m{5} & Find the output of LTI system having input signal
 $x[n]=\delta[n]+2\delta[n{-}1]-\delta[n{-}3]$ and $h[n]=2\delta[n{+}1]+2\delta[n{-}1]$.
 \newline {\color{sub}81 Bh prints the same pair, leading with $h$}
 & $\{2,\underline{4},2,2,0,-2\}$, from $n=-1$ \\
\hrow 79 Ba \m{5} & Find the output of LTI system having impulse response
 $h[n]=(\frac12)^n\{u[n{+}2]-u[n{-}2]\}$ to the input $x[n]=\{2,1,0,-1,4\}$.
 & $\{8,8,\underline{4},-2,14.5,7,3.5,2\}$, from $n=-2$ \\
74 Ash \m{3+2} & Find the output of LTI system having impulse response
 ...
 \mk{Also check the answer.} \newline {\color{sub}same $h$ as 79 Ba, different $x$}
...
\hrow 75 Ash \m{5} & Find the output of LTI system having impulse response
 $h[n]=2^n\{u[n]-u[n{-}3]\}$ and input signal
 $x[n]=\delta[n]+\delta[n{-}1]+\delta[n{-}2]$. & $\{\underline{1},3,7,6,4\}$ \\
\textbf{\texttt{79 Ch}}, \textbf{\texttt{75 Bh}} \m{4+6} & Define the recursive and
 non-recursive systems with examples. Find the output of LTI system having impulse
 response $h[n]=2^n\{u[n]-u[n{-}3]\}$ and input signal
 $x[n]=\delta[n]+\delta[n{-}1]-\delta[n{-}3]$. \newline {\color{sub}79 Ch prints
 \mk{"$2n$"} for $2^n$}
 & $\{\underline{1},3,6,3,-2,-4\}$ \\
\bottomrule
\end{tabularx}

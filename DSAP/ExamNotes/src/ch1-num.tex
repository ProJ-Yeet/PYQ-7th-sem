...
{\color{sub}\footnotesize \tF{6} \yr{\textbf{70 Ch} \m{6}, 73 Shr \m{6},
\textbf{\texttt{70 Bh}} \m{2+4}, 70 Asa \m{2+4}, \textbf{\texttt{74 Bh}} \m{6},
\texttt{74 Ma} \m{5}}
\gap$\cdot$\gap \mk{both sequences printed as plain sample lists}}

{\color{sub}\footnotesize \mk{Which convolution section am I in?} Decide from how the paper
\emph{writes} the two sequences, not from what it asks --- the instruction is the same
sentence every year.
\textbf{Both as sample lists} $\rightarrow$ \S1, straight to the product table.
\textbf{Either one as a sum of $\delta$'s} $\rightarrow$ \S1b: scale, shift, add, no table.
\textbf{$h$ as a windowed exponential} $a^n\{u[n{+}A]-u[n{-}B]\}$ $\rightarrow$ \S1c: write
the window out as samples first, then \S1.
\textbf{Both of those at once} $\rightarrow$ \S1d.
\textbf{Only the non-zero samples listed by index}, or drawn as stems $\rightarrow$ \S4.
A sequence that runs forever --- $u[n]$ or $a^nu[n]$ with no closing window --- is none of
these: that is \S2, and the answer there is a formula, not a list.}
...
{\color{sub}\footnotesize Five papers, and nothing changes but the digits: both sequences
are already sample lists starting at $n=0$, so the product table is written down
immediately.}
\par
\begin{tabularx}{\textwidth}{@{}L{2.0cm}XL{4.4cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed} & \thead{$y[n]$} \\
73 Shr \m{6} & Determine the system output $y(n)$ of the following signals:
 $h(n)=\{1,1,1\}$ and $x(n)=\{1,1,1,1\}$ & $\{\underline{1},2,3,3,2,1\}$ \\
\hrow \textbf{\texttt{70 Bh}} \m{2+4} & Illustrate the significance of convolution
 summation in digital signal analysis. Find the output sequence $y(n)$ if:
 $h(n)=\{1,1,1\}$ and \mk{$x(n)=\{1{-}2,2,3,4\}$} \newline
 {\color{sub}the comma after the 1 is missing; it is $\{1,-2,2,3,4\}$}
 & $\{\underline{1},-1,1,3,9,7,4\}$ \\
70 Asa \m{2+4} & Illustrate the significance of convolution summation in digital signal
 analysis. Compute the convolution of the following signals: $h(n)=\{1,0,1\}$ and
 $x(n)=\{1,-2,-2,3,4\}$ & $\{\underline{1},-2,-1,1,2,3,4\}$ \\
\hrow \textbf{\texttt{74 Bh}} \m{6} & Impulse response of an LTI system is given as
 $h(n)=\{2,2,-1,1\}$. Find the output $y(n)$ of the system to an input
 $x(n)=\{1,2,1,2\}$. & $\{\underline{2},6,5,5,5,-1,2\}$ \\
\texttt{74 Ma} \m{5} & Find the response of an \mk{FIR system} with input
 $x[n]=\{0,2,4,6\}$ and impulse response $h[n]=\{5,3,4,2,0\}$. \newline
 {\color{sub}$h$ and $x$ are named the other way round from every sibling, and both
 carry a \mk{zero at an end}: keep it, it is a real tap and it is why $y$ has 8 samples}
 & $\{\underline{0},10,26,50,38,32,12,0\}$ \\
\bottomrule
\end{tabularx}

\par\vspace{2pt}
{\color{sub}\footnotesize \mk{Convolution commutes}, so it never matters which sequence goes
across the top. Put the \emph{shorter} one down the side: fewer rows, fewer anti-diagonals to
read.}

\hr

\T{1b \quad One or both sequences written as impulses}
{\color{sub}\footnotesize \tP{3} \yr{\textbf{79 Bh} \m{5}, \textbf{81 Bh} \m{5},
\textbf{71 Ch} \m{6}}
\gap$\cdot$\gap \mk{no product table at all}: scale, shift, add}

\begin{asked}{2079 Bhadra \textperiodcentered{} Q2 \textperiodcentered{} [5]}
Find the output of LTI system having input signal $x[n]=\delta[n]+2\delta[n-1]-\delta[n-3]$
and $h[n]=2\delta[n+1]+2\delta[n-1]$.
\end{asked}
{\color{sub}\footnotesize \textbf{81 Bh} \m{5} sets the identical pair, leading with $h$:
``Find the output of an LTI System with impulse response $h[n]=2\delta[n+1]+2\delta[n-1]$
when an input of $x[n]=\delta[n]+2\delta[n-1]-\delta[n-3]$ is applied to it.''}

\lead{Method \mk{(one rule replaces the whole table)}}
\begin{enumerate}
  \item \textbf{Shifts add}:
        \[ \delta[n-a]*\delta[n-b]=\delta[n-a-b] \]
        This is the sifting property, and it is the only identity the section needs.
  \item Convolution is linear, so multiply the two sums out term by term: a coefficient
        $A$ at shift $a$ against a coefficient $B$ at shift $b$ contributes $AB$ at index
        $a+b$. Two impulses times three impulses is \textbf{six} contributions.
  \item Collect the contributions that land on the same index.
  \item \textbf{Fill every index in between with a zero}, including the ones nothing
        landed on.
  \item \mk{Check}: $\sum y=(\sum x)(\sum h)$, exactly as in \S1.
\end{enumerate}

\lead{Step 1: every pair of impulses}
\par
\begin{tabularx}{\textwidth}{@{}L{3.0cm}L{3.0cm}L{3.0cm}X@{}}
\toprule
\hrow \thead{from $x$} & \thead{from $h$} & \thead{lands at $a+b$} & \thead{contributes} \\
$1$ at $a=0$  & $2$ at $b=-1$ & $0+(-1)=-1$ & $1\times2=2$ \\
$1$ at $a=0$  & $2$ at $b=+1$ & $0+1=1$     & $1\times2=2$ \\
$2$ at $a=1$  & $2$ at $b=-1$ & $1-1=0$     & $2\times2=4$ \\
$2$ at $a=1$  & $2$ at $b=+1$ & $1+1=2$     & $2\times2=4$ \\
$-1$ at $a=3$ & $2$ at $b=-1$ & $3-1=2$     & $-1\times2=-2$ \\
$-1$ at $a=3$ & $2$ at $b=+1$ & $3+1=4$     & $-1\times2=-2$ \\
\bottomrule
\end{tabularx}

\lead{Step 2: collect by index}
\par
\begin{tabularx}{\textwidth}{@{}L{1.6cm}XL{2.2cm}@{}}
\toprule
\hrow \thead{$n$} & \thead{terms landing there} & \thead{$y[n]$} \\
$-1$ & $2$ & \textbf{2} \\
$0$ & $4$ & \textbf{4} \\
$1$ & $2$ & \textbf{2} \\
$2$ & $4-2$ & \textbf{2} \\
$3$ & \mk{nothing} & \textbf{0} \\
$4$ & $-2$ & \textbf{$-2$} \\
\bottomrule
\end{tabularx}

\lead{Final answer}
\[ \boxed{\;y[n]=\{2,\,\underline{4},\,2,\,2,\,0,\,-2\}\;}\qquad n=-1\ldots4 \]
{\color{sub}\footnotesize Check: $\sum x=1+2-1=2$, $\sum h=2+2=4$, $2\times4=8$; and
$2+4+2+2+0-2=8$ \checkmark \; \mk{The $n=3$ zero is not optional}: drop it and every sample
after it moves one place left, which loses the question.}

\lead{The same answer in one line, if you see it}
\[ y[n]=2x[n+1]+2x[n-1] \]
{\color{sub}\footnotesize $h$ is two impulses, so the output is two scaled copies of $x$, one
advanced by 1 and one delayed by 1, added. Written out: $2x[n+1]$ puts $\{2,4,0,-2\}$ at
$n=-1\ldots2$, $2x[n-1]$ puts $\{2,4,0,-2\}$ at $n=1\ldots4$, and the overlap at $n=1,2$ adds.
\mk{Same six numbers, no table} --- this is the 30-second route once the shift rule is
automatic.}

\lead{\textbf{71 Ch} \m{6}: impulses on one side, a negative start on the other}

\begin{asked}{2071 Chaitra \textperiodcentered{} Q2 \textperiodcentered{} [6]}
A discrete time LTI system has impulse response $h(n)=\{1,3,2,-1,1\}$ for $-1\le n\le3$.
Determine the system output $y(n)$ if the input $x(n)$ is given by
$x(n)=2\delta(n)-\delta(n-1)$.
\end{asked}

\sbs{%
\lead{Why this one is not \S1}
\begin{itemize}
  \item Only $h$ is a sample list, and it is the list that carries the \mk{marked start},
        $h[-1]=1$. $x$ is two impulses.
  \item So do not convert $x$ into a list and tabulate. Read $x$ as an \textbf{instruction
        on $h$}: one copy of $h$ scaled by 2, minus one copy delayed by 1.
  \item[] \[ y[n]=2h[n]-h[n-1] \]
  \item Shape first: start $=-1+0=-1$, length $=5+2-1=6$, so $n=-1\ldots4$.
  \item \mk{The delay row is where the marks go}. $h[n-1]$ is the $h$ row written one column
        to the \emph{right}, so its first entry sits at $n=0$, not $n=-1$.
\end{itemize}}{%
\lead{The whole calculation}
\par
\begin{tabular}{@{}L{1.9cm}L{0.8cm}L{0.8cm}L{0.8cm}L{0.8cm}L{0.8cm}L{0.8cm}@{}}
\toprule
\hrow \thead{$n$} & $-1$ & $0$ & $1$ & $2$ & $3$ & $4$ \\
$h[n]$        & 1 & 3 & 2 & $-1$ & 1 & 0 \\
$2h[n]$       & 2 & 6 & 4 & $-2$ & 2 & 0 \\
$h[n-1]$      & 0 & 1 & 3 & 2 & $-1$ & 1 \\
\hrow $y[n]$  & \textbf{2} & \textbf{5} & \textbf{1} & \textbf{$-4$} & \textbf{3}
 & \textbf{$-1$} \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\[ \boxed{\;y[n]=\{2,\,\underline{5},\,1,\,-4,\,3,\,-1\}\;}\quad n=-1\ldots4 \]
{\color{sub}\footnotesize Check: $\sum h=6$, $\sum x=2-1=1$, product $6$; and
$2+5+1-4+3-1=6$ \checkmark \newline
\mk{The trap}: $h$ starting at $-1$ is printed as a range, ``for $-1\le n\le3$'', not as an
underline. Miss that line and the whole answer is shifted one place and every sample is
wrong.}}

\hr

\T{1c \quad Impulse response given as a windowed exponential}
{\color{sub}\footnotesize \tF{4} \yr{79 Ba \m{5}, 74 Ash \m{3+2}, \textbf{82 Bh} \m{5},
72 Ka \m{5}}
\gap$\cdot$\gap \mk{one extra step before \S1}: write the window out as samples}

\begin{asked}{2079 Baishakh \textperiodcentered{} Q2 \textperiodcentered{} [5]}
Find the output of LTI system having impulse response
$h[n]=(1/2)^n\{u[n+2]-u[n-2]\}$ to the input $x[n]=\{2,1,0,-1,4\}$.
\end{asked}

\lead{Method \mk{(used for every problem in this section)}}
\begin{enumerate}
  \item \textbf{Turn the window into a tap count, before touching the exponential.}
        $u[n+A]-u[n-B]$ is 1 for $-A\le n\le B-1$: the lower limit is
        \emph{inclusive}, the upper limit is \mk{exclusive}.
        Here $u[n+2]-u[n-2]$ gives $-2\le n\le1$, which is \textbf{4} taps, not 5.
  \item \textbf{Evaluate $a^n$ at each of those $n$, negative $n$ included.}
        A negative index makes the sample \emph{larger}: $(1/2)^{-2}=4$.
        \[ h[n]=\{\,4,\;2,\;\underline{1},\;0.5\,\}\qquad n=-2\ldots1 \]
  \item From here it is \S1 exactly: product table, anti-diagonals, sum check.
  \item \textbf{Predict the shape with the real start index}: start $=n_x+n_h=0+(-2)=-2$,
        length $=5+4-1=8$, so $y[n]$ runs $n=-2$ to $5$.
\end{enumerate}

\lead{Step 1: product table \mk{($h$ is now an ordinary sample list)}}
\par
\begin{tabularx}{\textwidth}{@{}L{2.4cm}XXXXX@{}}
\toprule
\hrow & \thead{$x[0]=2$} & \thead{$x[1]=1$} & \thead{$x[2]=0$} & \thead{$x[3]=-1$}
      & \thead{$x[4]=4$} \\
$h[-2]=4$   & 8 & 4 & 0 & $-4$ & 16 \\
$h[-1]=2$   & 4 & 2 & 0 & $-2$ & 8 \\
$h[0]=1$    & 2 & 1 & 0 & $-1$ & 4 \\
$h[1]=0.5$  & 1 & 0.5 & 0 & $-0.5$ & 2 \\
\bottomrule
\end{tabularx}

\lead{Step 2: anti-diagonals, starting the count at $n=-2$}
\par
\begin{tabularx}{\textwidth}{@{}L{1.4cm}XL{2.2cm}@{}}
\toprule
...
\lead{Final answer}
\[ \boxed{\;y[n]=\{8,\,8,\,\underline{4},\,-2,\,14.5,\,7,\,3.5,\,2\}\;}\qquad n=-2\ldots5 \]
{\color{sub}\footnotesize Check: $\sum x=6$, $\sum h=4+2+1+0.5=7.5$, $6\times7.5=45$; and
$8+8+4-2+14.5+7+3.5+2=45$ \checkmark}

\lead{Every other paper that runs this same method}
{\color{sub}\footnotesize Read each row as: window $\rightarrow$ tap range $\rightarrow$
samples $\rightarrow$ \S1 table. Only the base $a$, the window edges and the input move.}
\par
\begin{tabularx}{\textwidth}{@{}L{2.0cm}XL{4.6cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed, and the window written out}
 & \thead{$y[n]$} \\
74 Ash \m{3+2} & Find the output of LTI system having impulse response
 $h[n]=(\frac12)^n\{u[n{+}2]-u[n{-}2]\}$ and input signal $x[n]=\{2,1,0.5,-1\}$.
 \mk{Also check the answer.} \newline
 {\color{sub}same $h$ as 79 Ba: $\{4,2,\underline{1},0.5\}$, $n=-2\ldots1$; only $x$ changed}
 & $\{8,8,\underline{6},-1,-1,-0.75,-0.5\}$, from $n=-2$ \newline
 {\color{sub}the check: $\sum x=2.5$, $\sum h=7.5$, $2.5\times7.5=18.75=\sum y$ \checkmark}
 \\
\hrow \textbf{82 Bh} \m{5} & Find the output of an LTI system whose impulse response is
 given by $0.5^n\{u[n]-u[n{-}3]\}$ and input is given by $\{2,1,0.5,-1\}$. \newline
 {\color{sub}window $0\le n\le2$, so \mk{no negative taps at all}:
 $h=\{\underline{1},0.5,0.25\}$ and $y$ starts at $n=0$}
 & $\{\underline{2},2,1.5,-0.5,-0.375,-0.25\}$ \\
72 Ka \m{5} & Find the output of LTI system having impulse response
 $h[n]=(\frac13)^n\{u[n{+}1]-u[n{-}2]\}$ and input signal $x[n]=\{2,1,0.5,3\}$. \newline
 {\color{sub}window $-1\le n\le1$, 3 taps: $h=\{3,\underline{1},\frac13\}$;
 $(\frac13)^{-1}=3$}
 & $\{6,\underline{5},\frac{19}{6},\frac{59}{6},\frac{19}{6},1\}$, from $n=-1$ \\
\bottomrule
\end{tabularx}

\par\vspace{2pt}
{\color{sub}\footnotesize \mk{The two ways this section is failed.} (1) Counting
$u[n+2]-u[n-2]$ as 5 taps ($n=-2\ldots2$) instead of 4: the upper step is exclusive, so
$n=2$ is already switched off. (2) Evaluating $(1/2)^{-2}$ as $1/4$ instead of $4$, which
makes the answer decay the wrong way --- \mk{a window reaching left of the origin always
starts with the largest sample} when $a<1$.}

\hr

\T{1d \quad Windowed exponential against an impulse comb}
{\color{sub}\footnotesize \tP{3} \yr{\textbf{\texttt{79 Ch}} \m{4+6},
\textbf{\texttt{75 Bh}} \m{4+6}, 75 Ash \m{5}}
\gap$\cdot$\gap \S1c on one side, \S1b on the other, \mk{and the fast route uses both}}

\begin{asked}{2079 Chaitra \textperiodcentered{} Q2 \textperiodcentered{} [4+6]}
Define the recursive and non-recursive systems with examples. Find the output of LTI system
having impulse response $h[n]=2^n\{u[n]-u[n-3]\}$ and input signal
$x[n]=\delta[n]+\delta[n-1]-\delta[n-3]$.
\end{asked}
{\color{sub}\footnotesize \textbf{\texttt{75 Bh}} \m{4+6} prints the same question word for
word. \texttt{79 Ch} prints \mk{``$2n$''} where it means $2^n$: the window makes it an
exponential, and a linear $2n$ would make $h[0]=0$, which no answer key uses.}

\lead{Method \mk{(both sides convert, then it is four lines)}}
\begin{enumerate}
  \item \textbf{Window $\rightarrow$ samples} (\S1c). $u[n]-u[n-3]$ is 1 for
        $0\le n\le2$, so
        \[ h[n]=\{\,\underline{1},\;2,\;4\,\}\qquad n=0,1,2 \]
        \mk{Here $a=2>1$, so the taps grow} instead of decaying.
  \item \textbf{Impulses $\rightarrow$ shifts} (\S1b). $x[n]=\delta[n]+\delta[n-1]-\delta[n-3]$
        says: one copy of $h$, plus a copy delayed by 1, minus a copy delayed by 3.
        \[ y[n]=h[n]+h[n-1]-h[n-3] \]
  \item Lay the three shifted copies in a grid and add the columns. \textbf{No product
        table is needed}, because every coefficient of $x$ is $\pm1$.
  \item Shape: start $=0+0=0$, length $=3+4-1=6$, so $n=0\ldots5$. \mk{$x$ has length 4},
        not 3 --- the missing $\delta[n-2]$ is a zero sample inside the sequence, and it
        still counts towards the length.
\end{enumerate}

\lead{The calculation: three shifted copies of $h$, added down the columns}
\par
\begin{tabularx}{\textwidth}{@{}L{2.6cm}L{1.3cm}L{1.3cm}L{1.3cm}L{1.3cm}L{1.3cm}X@{}}
\toprule
\hrow \thead{$n$} & $0$ & $1$ & $2$ & $3$ & $4$ & $5$ \\
$h[n]$          & 1 & 2 & 4 & 0 & 0 & 0 \\
$h[n-1]$        & 0 & 1 & 2 & 4 & 0 & 0 \\
$-h[n-3]$       & 0 & 0 & 0 & $-1$ & $-2$ & $-4$ \\
\hrow $y[n]$ & \textbf{1} & \textbf{3} & \textbf{6} & \textbf{3} & \textbf{$-2$}
 & \textbf{$-4$} \\
\bottomrule
\end{tabularx}

\lead{Final answer}
\[ \boxed{\;y[n]=\{\underline{1},\,3,\,6,\,3,\,-2,\,-4\}\;}\qquad n=0\ldots5 \]
{\color{sub}\footnotesize Check: $\sum h=1+2+4=7$, $\sum x=1+1-1=1$, product $7$; and
$1+3+6+3-2-4=7$ \checkmark \; \mk{Read the sign change}: the output is positive while the
first two copies dominate and turns negative from $n=4$, where only the subtracted copy is
left. A sign flip in the tail is the signature of a $-\delta$ in the input.}

\lead{75 Ash \m{5}: the same grid with three plus signs}

\begin{asked}{2075 Ashwin \textperiodcentered{} Q2 \textperiodcentered{} [5]}
Find the output of LTI system having impulse response $h[n]=2^n\{u[n]-u[n-3]\}$ and input
signal $x[n]=\delta[n]+\delta[n-1]+\delta[n-2]$.
\end{asked}

\sbs{%
\lead{The grid}
\par
\begin{tabular}{@{}L{1.9cm}L{0.9cm}L{0.9cm}L{0.9cm}L{0.9cm}L{0.9cm}@{}}
\toprule
\hrow \thead{$n$} & $0$ & $1$ & $2$ & $3$ & $4$ \\
$h[n]$       & 1 & 2 & 4 & 0 & 0 \\
$h[n-1]$     & 0 & 1 & 2 & 4 & 0 \\
$h[n-2]$     & 0 & 0 & 1 & 2 & 4 \\
\hrow $y[n]$ & \textbf{1} & \textbf{3} & \textbf{7} & \textbf{6} & \textbf{4} \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\[ \boxed{\;y[n]=\{\underline{1},\,3,\,7,\,6,\,4\}\;}\quad n=0\ldots4 \]
{\color{sub}\footnotesize Check: $\sum h=7$, $\sum x=3$, $7\times3=21$; and
$1+3+7+6+4=21$ \checkmark}}{%
\lead{Why the two answers differ in length}
\begin{itemize}
  \item 75 Ash: $x$ is $\{1,1,1\}$, length 3, so $y$ has $3+3-1=\mathbf{5}$ samples.
  \item 79 Ch: $x$ is $\{1,1,0,-1\}$, length \textbf{4} --- the gap at $n=2$ is inside the
        sequence --- so $y$ has $3+4-1=\mathbf{6}$.
  \item \mk{Three impulses do not always mean length 3.} The length runs from the first
        non-zero index to the last, and counts every index in between.
  \item Both answers rise to a peak and then follow $h$'s own shape: the running sum of a
        growing exponential is dominated by its newest term, which is why $y$ keeps
        climbing while all three copies overlap.
\end{itemize}}
...
\par
\begin{tabularx}{\textwidth}{@{}L{2.1cm}XL{6.1cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed} & \thead{Answer} \\
\texttt{73 Ma} \m{2+6} & Define a Causal and Stable system in terms of impulse response.
 Find the output of LTI system having impulse response $h[n]=u[n{+}1]-u[n{-}4]$ and
 input signal $x[n]=(\frac12)^nu[n]$. \newline {\color{sub}5 taps, $-1\ldots3$}
 & $y=2-(\frac12)^{n+1}$ for $-1\le n\le3$; \; $y=31(\frac12)^{n+1}$ for $n\ge4$ \\
\hrow \textbf{\texttt{78 Ch}} \m{6} & Find the output of LTI system having impulse
 response $h[n]=u[n{+}1]-u[n{-}3]$ and input signal $x[n]=(0.5)^nu[n]$. \newline
 {\color{sub}4 taps, $-1\ldots2$}
 & $y=2-(\frac12)^{n+1}$ for $-1\le n\le2$; \; $y=15(\frac12)^{n+1}$ for $n\ge3$ \\
\textbf{76 Ch} \m{6} & Find the output of LTI system having input signal
 $x[n]=u[n{+}1]-u[n{-}4]$ and impulse response $h[n]=(\frac12)^nu[n{-}1]$. \newline
 {\color{sub}\mk{the roles are swapped}: here the step is $x$, not $h$}
 & $y=1-(\frac12)^{n+1}$ for $0\le n\le4$; \; $y=31(\frac12)^{n+1}$ for $n\ge5$ \\
\hrow \textbf{69 Ch} \m{1+4} & Define LTI system. Find the output of LTI system having
 impulse response $h[n]=2u[n]-2u[n{-}4]$ and input signal $x[n]=(\frac13)^nu[n]$.
 & $y=3\left[1-(\frac13)^{n+1}\right]$ for $0\le n\le3$; \; $y=80(\frac13)^{n}$ for
 $n\ge4$ \\
\textbf{\texttt{81 Ch}} \m{2+2+6}, \textbf{\texttt{80 Ch}} \m{2+2+6},
 \texttt{72 Ma} \m{8} & Define a Causal and Stable system in terms of impulse response.
 Find the output of LTI system having impulse response $h[n]=2u[n]-2u[n{-}5]$ and input
 signal $x[n]=(\frac13)^nu[n]$. \newline {\color{sub}72 Ma prints the design alone, for
 all 8 marks}
 & $y=3\left[1-(\frac13)^{n+1}\right]$ for $0\le n\le4$; \; $y=242(\frac13)^{n}$ for
 $n\ge5$ \\
\hrow \textbf{\texttt{76 Bh}} \m{2+6} & Differentiate between FIR and IIR system. Find
 the output of LTI system having impulse response $h[n]=2u[n{+}2]-2u[n{-}3]$ and input
 signal $x[n]=(\frac13)^nu[n{-}1]$. \newline {\color{sub}5 taps, $-2\ldots2$, and $x$
 starts at $n=1$}
 & $y=1-(\frac13)^{n+2}$ for $-1\le n\le3$; \; $y=242(\frac13)^{n+2}$ for $n\ge4$ \\
\textbf{\texttt{77 Ch}} \m{3+7} & Define causality and stability of LTI system in terms
 of impulse response with a suitable example. Find the output of LTI system having
 impulse response $h[n]=2u[n{+}3]-2u[n{-}4]$ and input signal
 $x[n]=(\frac12)^nu[n{+}1]$. \newline {\color{sub}7 taps, $-3\ldots3$, and $x$ starts at
 $n=-1$: \mk{the widest of the family}}
...
\lead{Every other paper that runs this same method}
\par
\begin{tabularx}{\textwidth}{@{}L{2.2cm}XL{3.0cm}L{3.3cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed} & \thead{$H(e^{j\omega_0})$}
& \thead{$y[n]$} \\
\textbf{70 Ch} \m{4} & Find the output of LTI System having impulse response
 $h[n]=(\frac13)^nu[n]$ and input signal $x[n]=5e^{j\pi n/2}$ for
 $-\infty<n<\infty$.
 & $\frac{1}{1+j/3}=0.9-j0.3$ \newline $=0.9487\angle-18.43^\circ$
 & $4.7434\,e^{j(\pi n/2-0.3217)}$ \\
\hrow 72 Ka \m{4}, 80 Ba \m{5}, 82 Ba \m{5} & Find the output of LTI system having
 impulse response $h[n]=(\frac12)^n\,u[n]$ and input $x[n]=5e^{j\pi n/3}$
 \newline {\color{sub}80 Ba adds "for $-\infty<n<\infty$"; 82 Ba says "excited by an
 input"}
 & $1.1547\angle-30^\circ$ & $5.7735\,e^{j(\pi n/3-\pi/6)}$ \\
\textbf{\texttt{69 Bh}} \m{2+4} & Why do you need \mk{different equation}? Consider an
 LTI system with impulse response $h[n]=(\frac12)^nu[n]$. Determine $y[n]$, the output
 of this system, if the input is $x[n]=Ae^{jn\pi/2}$ \newline
 {\color{sub}"different" for "difference"}
 & $\frac{1}{1+j0.5}$ \newline $=0.8944\angle-26.57^\circ$
 & $0.8944A\,e^{j(\pi n/2-0.4636)}$ \\
\hrow 71 Shr \m{2+3+5} & Why do we need difference equation? State linear constant
 coefficient difference equation and corresponding system function. Consider an LTI
 system with impulse response $h[n]=(\frac12)^nu[n]$. Determine $y[n]$, if the input is
 $x[n]=Ae^{jn\pi}$
 & $\frac{1}{1+\frac12}=\frac23$ \newline (real, $e^{-j\pi}=-1$)
 & $\frac23A\,e^{j\pi n}$ \\
\bottomrule
\end{tabularx}
...
\lead{Every other paper that runs this same method}
\par
\begin{tabularx}{\textwidth}{@{}L{2.0cm}XL{5.5cm}@{}}
\toprule
\hrow \thead{Paper} & \thead{Question as printed} & \thead{$y[n]$} \\
\textbf{75 Ch} \m{5+2} & Find the output of LTI system having impulse response $h[n]$
 with $h[-2]=3$, $h[0]=2$, $h[1]=1$ and input signal $x[n]=(2)^n$, for
 $-1\le n\le3$. \mk{Also check the answer.}
 & $\{1.5,3,7,\underline{14.5},29,10,20,8\}$, from $n=-3$ \\
\hrow \textbf{72 Ch} \m{3+2} & Find the output of LTI system having impulse response
 $h[n]$ with $h[-2]=1$, $h[0]=2$, $h[1]=3$ and input signal $x[n]$ with $x[0]=1/2$,
 $x[2]=2$, $x[3]=3$. \mk{Also check the answer.} \newline {\color{sub}\mk{every
 unlisted sample is zero}, including $x[1]$}
 & $\{0.5,0,\underline{3},4.5,4,12,9\}$, from $n=-2$ \\
\textbf{\texttt{71 Bh}} \m{7} & Find the output sequence $y[n]$ \mk{and verify it} for
 the two stem plots given: $h[0]=0.5$, $h[1]=0.75$, $h[2]=1$; $x[-2]=1$, $x[0]=0.5$,
 $x[2]=1$, $x[3]=0.75$. \newline {\color{sub}both sequences are drawn, not written}
 & $\{0.5,0.75,\underline{1.25},0.375,1,1.125,1.5625,0.75\}$, from $n=-2$ \\
\bottomrule
\end{tabularx}

\par\vspace{2pt}
\lead{\textbf{75 Ch} \m{5+2}: the input is a \emph{formula} over a stated range}

\begin{asked}{2075 Chaitra \textperiodcentered{} Q2 \textperiodcentered{} [5+2]}
Find the output of LTI system having impulse response $h[n]$ with $h[-2]=3$, $h[0]=2$,
$h[1]=1$ and input signal $x[n]=(2)^n$, for $-1\le n\le3$. Also check the answer.
\end{asked}

\sbs{%
\lead{The extra step, and the trap inside it}
\begin{itemize}
  \item $h$ is listed by index, so fill the gap first: $h[-1]$ is unlisted, therefore zero.
  \item[] \[ h[n]=\{\,3,\;0,\;\underline{2},\;1\,\}\qquad n=-2\ldots1 \]
  \item $x$ is \mk{not a list at all}. It is $2^n$ \emph{evaluated} over the range the paper
        states, so generate the samples before anything else.
  \item[] \[ x[n]=\{\,0.5,\;\underline{1},\;2,\;4,\;8\,\}\qquad n=-1\ldots3 \]
  \item \mk{Where the marks go}: reading $x$ as $\{1,2,4,8,16\}$ from $n=0$. The range
        opens at $-1$, so the first sample is $2^{-1}=0.5$ and it sits \emph{left} of the
        origin --- the same negative-exponent step as \S1c.
  \item Shape: start $=n_x+n_h=-1+(-2)=-3$, length $=5+4-1=8$, so $n=-3\ldots4$.
  \item From here it is \S1 unchanged: product table, anti-diagonals, sum check.
\end{itemize}}{%
\lead{Product table, then the check the question demands}
\par
\begin{tabular}{@{}L{1.55cm}L{1.05cm}L{0.95cm}L{0.95cm}L{0.95cm}L{0.95cm}@{}}
\toprule
\hrow & \thead{$0.5$} & \thead{$1$} & \thead{$2$} & \thead{$4$} & \thead{$8$} \\
$h[-2]=3$ & 1.5 & 3 & 6 & 12 & 24 \\
$h[-1]=0$ & 0 & 0 & 0 & 0 & 0 \\
$h[0]=2$  & 1 & 2 & 4 & 8 & 16 \\
$h[1]=1$  & 0.5 & 1 & 2 & 4 & 8 \\
...
\end{tabular}
\par\vspace{3pt}
\[ \boxed{\;y[n]=\{1.5,\,3,\,7,\,\underline{14.5},\,29,\,10,\,20,\,8\}\;} \]
\vspace{-4pt}
\begin{itemize}
  \item $n=-3\ldots4$, and the underline marks $n=0$.
  \item \textbf{``Also check the answer''}, worth the 2 marks: $\sum x=15.5$,
        $\sum h=6$, $15.5\times6=93$; and
        $1.5+3+7+14.5+29+10+20+8=93$ \checkmark
  \item \mk{The zero row is not decoration}. Leaving $h[-1]$ out drops $h$ to three taps,
        shortens $y$ to seven samples and moves every one of them.
\end{itemize}}
...
\lead{74 Ash, 79 Ba and \textbf{82 Bh} \m{2+2}: a complex exponential, where
$P=A^2$ and \mk{not} $A^2/2$}

\sbs{%
\lead{Where the factor of 2 goes}
\begin{itemize}
  \item 74 Ash prints $x[n]=e^{j(\pi n/3+\pi/4)}$; 79 Ba and \textbf{82 Bh} print
        $x[n]=e^{j(\pi n/2+4\pi/7)}$. Same working, different $\omega_0$.
  \item $|e^{j\theta}|=1$ for \emph{every} real $\theta$, so $|x[n]|^2=1$ at every single
        $n$. \mk{There is nothing to average}, which is the whole difference from a
        cosine.
  \item[] \[ E=\sum_{n=-\infty}^{\infty}1=\boxed{\;\infty\;} \]
  \item[] \[ P=\lim_{N\to\infty}\frac{1}{2N+1}\sum_{n=-N}^{N}1
             =\lim_{N\to\infty}\frac{2N+1}{2N+1}=\boxed{\;1\;} \]
  \item $E=\infty$ with $P$ finite and non-zero, so it is a \textbf{power signal}.
        \mk{Every sample counts here}, unlike $u[n]$, where only $N+1$ of the $2N+1$ are
        non-zero and the limit lands on $\tfrac12$.
\end{itemize}}{%
\lead{The three power formulas, side by side}
\begin{itemize}
  \item $A\cos(\omega_0n+\phi)$: \; $P=\dfrac{A^2}{2}$, because
        $\overline{\cos^2}=\tfrac12$ over a period.
  \item $Ae^{j(\omega_0n+\phi)}$: \; $P=A^2$, because the magnitude never moves.
  \item Two sinusoids at \emph{different} frequencies: the powers \textbf{add}, which is
        the 79 Bh signal worked above.
  \item \mk{The constant phase is a decoy.} $\pi/4$ and $4\pi/7$ shift the signal, and
        neither $E$, $P$ nor the period depends on a shift: do not expand the exponential
        to get rid of it.
  \item The second half of each of these papers is \S6: $\omega_0=\pi/3$ gives $N=6$,
        $\omega_0=\pi/2$ gives $N=4$.
\end{itemize}}

\lead{Every other paper that runs this same method}
\par
{\color{sub}\footnotesize \mk{The instruction varies and the marks follow it.}
Papers print one of: "Compare between energy signal and power signal. Determine whether
the signal \ldots is energy signal or power signal"; "Define energy and power signal.
Check the signal \ldots is Energy or Power type"; or "Define energy and power signal.
Determine whether the signal \ldots is periodic or non-periodic and if it is periodic,
find its fundamental period". \mk{The last one is two questions}, and its second half is
\S6. The definition is always worth 2 marks on its own.}
\par\smallskip
...
\lead{Every other paper that runs this same method}
\par
{\color{sub}\footnotesize \mk{Every one of these prints the same two-part
instruction}, in one of these forms: "Determine if the signal \ldots is periodic or not.
If the signal is periodic, find its fundamental period"; "Determine whether the given
signal is periodic or not. If the signal is periodic, determine the fundamental period";
"Check whether following signals are periodic or not. If yes, state their periodic
time"; or "Determine which of the following signals are periodic and compute their
fundamental period". \mk{Answering only "periodic" loses half the marks} every time.}
\par\smallskip
...
\par
{\color{sub}\footnotesize The instruction, as the papers print it: "State whether
or not the system \ldots is (a) linear (b) time invariant (c) memoryless (d) causal
(e) stable", or "Determine whether the following systems are (a) causal (b) linear", or
"Check whether the following system is linear or non-linear, time variant or
invariant". \mk{Name every property the paper lists and no more}: the marks are split
one per property, so a full proof of two of five is worth two marks.}
\par\smallskip

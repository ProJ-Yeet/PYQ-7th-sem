...
\lead{Every other direct circular convolution, question as printed}
{\color{sub}\footnotesize Build the circulant of the second sequence and multiply. The
only decision is $N$, and \mk{when the paper does not say, $N=\max(L,M)$}, with the
shorter sequence zero padded up to it.}
\par\smallskip
{\footnotesize
\begin{tabular}{@{}L{2.5cm}L{10.3cm}L{0.5cm}L{3.4cm}@{}}
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$N$} & \thead{$y[n]$} \\
81 Ba & Compute circular convolution of the following two sequences using DFT.
  $x(n)=\{1,2,4,5\}$ and $h(n)=\{2,1,6,8\}$ \m{6} & 4 & \{$47$, $67$, $56$, $34$\} \\
\textbf{72 Ch}, \textbf{82 Bh} & Compute Circular Convolution of $h(n)=\{1,2,1,-1,1\}$
  and $x[n]=\{1,2,3,1\}$ \m{7} \newline {\color{sub}82 Bh prints the same pair, $h$
  first again} \m{7} & 5 & \{$1$, $6$, $9$, $8$, $4$\} \\
79 Ba & Find the circular convolution of the sequences $x_1[n]=\{1,-1,-2,3,-1\}$ and
  $x_2[n]=\{1,2,3\}$. \m{7} & 5 & \{$8$, $-2$, $-1$, $-4$, $-1$\} \\
82 Ba & Find the circular convolution of the sequences $x_1[n]=\{1,1,-1,-1,2\}$ and
  $x_2[n]=\{1,-1,-2\}$. \m{6} & 5 & \{$1$, $-4$, $-4$, $-2$, $5$\} \\
\textbf{78 Bh}, \texttt{74 Ma} & Differentiate between DFT and DTFT. Find the circular
  convolution of $x_1[n]=\{2,1,2,1\}$ and $x_2[n]=\{1,2,3,4\}$ \m{2+6}\m{1+5}
  & 4 & \{$14$, $16$, $14$, $16$\} \\
\textbf{\texttt{78 Ch}} & Find the circular convolution for $x_1[n]=\{2,1,2,1\}$ and
  $x_2[n]=\{1,-2,-1,3\}$. \m{5} & 4 & \{$1$, $2$, $1$, $2$\} \\
\bottomrule
\end{tabular}}
\par\smallskip
{\footnotesize
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$N$} & \thead{$y[n]$} \\
75 Ash & State the circular convolution property of DFT. Find the circular convolution
  of: $x_1(n)=\{1,2,-1,1\}$ and $x_2(n)=\{1,3,5,7\}$ \m{1+5}
  & 4 & \{$13$, $3$, $17$, $15$\} \\
\textbf{75 Ch} & Differentiate between linear convolution and circular convolution
  compute circular convolution of signals $X_1[n]=\{0,0,1,1\}$ and $X_2[n]=\{1,1,1,1\}$
  \m{3+7} & 4 & \{$2$, $2$, $2$, $2$\} \\
\textbf{\texttt{76 Bh}} & Determine the circular convolution of the following sequences
  $x_1[n]=\{1,0.5,1,0.5\}$ and $x_2[n]=\{0,1,2,3\}$. \m{6}
  & 4 & \{$4$, $5$, $4$, $5$\} \\
\textbf{\texttt{71 Bh}} & Perform circular convolution of: $x_1[n]=[1\ 2\ 0\ 3\ 4]$,
  $x_2[n]=[2\ {-}1\ 2\ {-}1\ 2]$ \m{5} \newline {\color{sub}square brackets and no
  commas, which is this paper's house style} & 5
  & \{$8$, $8$, $2$, $17$, $5$\} \\
\textbf{67 Mng} & Perform circular convolution of the sequences $x_1[n]=[1,2,1]$,
  $0\le n\le2$ and $x_2[n]=[1,2,0,1]$, $0\le n\le3$. \m{5} & 4
  & \{$3$, $5$, $5$, $3$\} \\
\textbf{66 Ma} & Perform circular convolution of the sequences $x_1(n)=[1,2]$,
  $0\le n\le1$ and $x_2(n)=[1,3,4,5]$, $0\le n\le3$. \m{4} & 4
  & \{$11$, $5$, $10$, $13$\} \\
\textbf{69 Bh} & Perform circular convolution of the sequences $x[n]=[1\ 0\ 1]$ and
  $h[n]=[1\ 0\ 2\ 1]$. \m{5} & 4 & \{$3$, $1$, $3$, $1$\} \\
\textbf{71 Ch}, 70 Asa, \textbf{\texttt{79 Ch}} & Why do we need DFT although we have
  DTFT? Find circular convolution between $x[n]=\{1,2\}$ and \mk{$y[n]=u[n]-u[n-4]$}
  \m{2+5}\m{2+5}\m{3+5} \newline {\color{sub}$y$ is a formula: it is
  $\{1,1,1,1\}$} & 4 & \{$3$, $3$, $3$, $3$\} \\
\bottomrule
\end{tabular}}
\par\smallskip
\begin{itemize}
  \item \mk{Three of these hand you $N$ implicitly.} 67 Mng, 66 Ma and 69 Bh print
        $0\le n\le3$ on the longer sequence, so $N=4$; 71 Ch, 70 Asa and 79 Ch print a
        step difference that has to be expanded to $\{1,1,1,1\}$ first.
  \item \mk{75 Ch's and 71 Ch's answers are constant} because their second sequence is
        constant: convolving anything with a flat sequence circularly puts $\sum x_1$ in
        every position. $\sum\{0,0,1,1\}=2$ and $\sum\{1,2\}=3$. Free marks once seen.
  \item \mk{82 Bh and 72 Ch print $h$ before $x$.} Circular convolution commutes, so it
        changes nothing, but say so rather than silently swapping them.
\end{itemize}
...
\par\smallskip
{\footnotesize
\begin{tabular}{@{}L{2.4cm}L{10.4cm}L{0.5cm}L{3.4cm}@{}}
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$N$} & \thead{$x_3[n]$} \\
\textbf{76 Ch}, \textbf{69 Ch} & Find $x_3[n]$ if DFT of $x_3[n]$ is given by
  $X_3(k)=X_1(k)*X_2(k)$ where $X_1(k)$ and $X_2(k)$ are 4-point DFT of
  $x_1[n]=\{1,2,-2\}$ and $x_2[n]=\{1,2,3,-1\}$ respectively. \m{5}\m{6} \newline
  {\color{sub}76 Ch prints a $*$ between the two DFTs; it means the ordinary product,
  not convolution} & 4 & \{$-7$, $6$, $5$, $1$\} \\
\texttt{72 Ma} & Find $x_3[n]$ if DFT of $x_3[n]$ is given by $X_3(k)=X_1(k)X_2(k)$
  where $X_1(k)$ and $X_2(k)$ are 4-point DFT of $x_1[n]=\{1,2,3,-1\}$ and
  $x_2[n]=\{2,1,-3\}$ respectively. \m{2+4} & 4 & \{$-8$, $8$, $5$, $-5$\} \\
\textbf{79 Bh} & If $X_1(k)$ and $X_2(k)$ are DFT of sequence $x_1[n]=\{1,0,0,1\}$ and
  $x_2[n]=\{2,0,2\}$ respectively then find the sequence $x_3[n]$; if DFT of $x_3[n]$
  is given by $X_3(k)=X_1(k)\cdot X_2(k)$ \m{7} & 4 & \{$2$, $2$, $2$, $2$\} \\
72 Ka & Why we need DFT? If $X_1(k)$ and $X_2(k)$ are DFT of sequence
  $x_1[n]=\{1,2,4\}$ and $x_2[n]=\{-1,2,3,1\}$ respectively, then find the sequence
  $x_3[n]$, if DFT of $x_3[n]$ is given by $X_3(k)=X_1(k)X_2(k)$. \m{2+6}
  & 4 & \{$13$, $4$, $3$, $15$\} \\
74 Ash & What is a zero padding? If $X_1(k)$ and $X_2(k)$ are DFT of sequence
  $x_1[n]=\{1,2,0,1,-2\}$ and $x_2[n]=\{1,0,1,1,2\}$ respectively then find the
  sequence $x_3[n]$; If DFT of $x_3[n]$ is given by $X_3(k)=X_1(k),X_2(k)$. \m{1+7}
  \newline {\color{sub}the comma between the two DFTs is a printing slip for a product}
  & 5 & \{$6$, $1$, $1$, $0$, $2$\} \\
71 Shr & A system has input signal $x[n]=\{1,2,3,4\}$ and impulse response
  $h[n]=\{1,3,5,7\}$ and the DFT of $x[n]$ is $X[k]$ and the DFT of $h[n]$ is $H[k]$.
  Find the output of the system $y[n]$ if $G[k]=X[k].H[k]$ \m{7} \newline
  {\color{sub}\mk{the same question wearing a system's clothes}} & 4
  & \{$42$, $46$, $42$, $30$\} \\
\bottomrule
\end{tabular}}
\par\smallskip
...
\toprule
\hrow \thead{Papers} & \thead{Question as printed} & \thead{$N$} & \thead{$x_3[n]$} \\
\textbf{81 Bh}, \textbf{\texttt{81 Ch}}, \textbf{\texttt{75 Bh}} & If $X_1(k)$ and
  $X_2(k)$ are the 5-point DFT of $x_1(n)=3^n$ $0\le n\le3$ and $x_2(n)=2^n$
  $0\le n\le4$. Find $x_3(n)$ if $X_3(k)=X_1(k)X_2(k)$ \m{6} \newline
  {\color{sub}\mk{81 Ch says 4-point and 75 Bh says nothing}; see the defect note
  below} \m{1+6}\m{1+5} & 5 & \{$229$, $365$, $451$, $65$, $130$\} \\
\textbf{73 Ch} & Find $x_3[n]$ if DFT of $x_3[n]$ is given by $X_3(k)=X_1(k)X_2(k)$
  where $X_1(k)$ and $X_2(k)$ are 5-point DFT of $x_1[n]=\{1,-2,5,1,2\}$ and
  $x_2[n]=\{1,2,-3,-2\}$ respectively. \m{7} & 5
  & \{$-8$, $-8$, $-6$, $15$, $-7$\} \\
73 Shr & Find $x_3[n]$ if DFT of $x_3[n]$ is given by $X_3(k)=X_1(k)X_2(k)$ where
  $X_1(k)$ and $X_2(k)$ are 5-point DFT of $x_1[n]=\{1,-2,2,1,4\}$ and
  $x_2[n]=\{2,1,-3,-1\}$ respectively. \m{7} & 5
  & \{$1$, $-16$, $-5$, $9$, $5$\} \\
\textbf{\texttt{80 Ch}}, \textbf{\texttt{73 Bh}} & Find $x_3[n]$ if DFT of $x_3[n]$ is
  given by $X_3(k)=X_1(k)X_2(k)$ where $X_1(k)$ and $X_2(k)$ are 5-point DFT of
  $x_1[n]=\{1,2,3,-1,5\}$ and $x_2[n]=\{2,1,-3\}$ respectively. \m{7}\m{1+5} \newline
  {\color{sub}73 Bh prefixes "State Multiplication of two DFTs property of DFT"}
  & 5 & \{$10$, $-10$, $5$, $-5$, $0$\} \\
\texttt{73 Ma} & Find $x_3[n]$ if DFT is given by $x_3(k)=x_1(k)x_2(k)$ where
  $x_1[n]=\{3,2,1,-1,-2\}$ and $x_2[n]=\{1,2,3\}$ for 5 points DFT. \m{6}
  & 5 & \{$-4$, $2$, $14$, $7$, $-1$\} \\
\bottomrule
\end{tabular}}

\begin{itemize}
  \item \mk{71 Shr is the same question in disguise}: "input $x[n]$, impulse response
        $h[n]$, find $y[n]$ if $G[k]=X[k]H[k]$". It is the 4-point \textbf{circular}
  ...
  \item \mk{74 Ash, not 74 Ch.} This question is 2074 \textbf{Ashwin} Q10, a Back paper.
        Both this chapter and the Sorted PYQ document tagged it 74 Ch for years; 2074
        Chaitra asks no product-of-DFTs question at all. Corrected 2026-09-15.
\end{itemize}
